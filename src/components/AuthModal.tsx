...
export default function AuthModal() {
  ...
  const { authModalOpen, setAuthModalOpen, setRoleModalOpen } = useNav();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      setInfo(null);
      setTimeout(() => setRoleModalOpen(true), 200);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm animate-fade-in" onClick={() => setAuthModalOpen(false)} />
      <div className="relative w-full max-w-md animate-scale-in rounded-2xl bg-white shadow-2xl dark:bg-brand-900">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-brand-800">
          ...
          <button onClick={() => setAuthModalOpen(false)} className="rounded-full border border-gray-300 p-2 text-gray-400 transition-colors hover:border-btnblue-400 hover:text-btnblue-600 dark:border-brand-700 dark:hover:border-btnblue-500">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="John Doe" className="input-field pl-10" />
                </div>
              </div>
            )}
            <div>
                  ...
                  <div className="mt-3 space-y-1.5">
                    <div className="flex gap-1">
                      {PASSWORD_RULES.map((rule, i) => (
                        <div key={i} className={cn('h-1.5 flex-1 rounded-full transition-colors', i < passwordStrength ? 'bg-success-500' : 'bg-gray-200 dark:bg-brand-800')} />
                      ))}
                    </div>
                    <ul className="space-y-1">
                      {PASSWORD_RULES.map((rule) => {
                        ...
                        return (
                          <li key={rule.label} className={cn('flex items-center gap-1.5 text-xs', passed ? 'text-success-600 dark:text-success-400' : 'text-gray-400')}>
            ...
            {info && <div className="rounded-lg bg-success-50 px-4 py-2.5 text-sm text-success-700 dark:bg-success-900/20 dark:text-success-400">{info}</div>}

            {mode === 'signin' && (
              <div className="text-right">
                <button type="button" onClick={() => switchMode('forgot')} className="text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">
                  Forgot password?
                </button>
              </div>
            )}
          ...
          <p className="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <button onClick={() => switchMode(mode === 'signin' ? 'signup' : 'signin')} className="font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400">
        ...
        <div className="border-t border-gray-200 px-6 py-4 dark:border-brand-800">
          <p className="text-center text-xs text-gray-500 dark:text-gray-400">© Copyright Saka Krib. All Rights Reserved.</p>
        </div>
      </div>
    </div>
  );
