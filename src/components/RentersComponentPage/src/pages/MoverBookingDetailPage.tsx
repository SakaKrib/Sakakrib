...
export default function MoverBookingDetailPage() {
  ...
  const [data, setData] = useState<MoverBookingDetailResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [actionLoading, setActionLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);
  const [decision, setDecision] = useState<'confirm' | 'not_sure' | 'cancel' | null>(null);
  ...
  const loadBooking = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setData(result as MoverBookingDetailResponse);
      } catch (err) {
        console.error('Failed to load mover booking:', err);
        setError(getRpcErrorMessage(err, 'Unable to load this moving booking.'));
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
  ...
  useEffect(() => {
    if (!data?.response_deadline || !data.can_respond) return;
    const interval = window.setInterval(() => setNow(Date.now()), 1000);
    return () => window.clearInterval(interval);
  }, [data?.response_deadline, data?.can_respond]);

  const deadline = data?.response_deadline
    ? new Date(data.response_deadline).getTime()
    : null;
  ...
  const handleDecision = async () => {
    ...
    if ((decision === 'cancel' || decision === 'not_sure') && !reason.trim()) {
      setActionError(
        decision === 'cancel'
          ? 'Please provide a reason for declining this request.'
          : 'Please explain what you need to clarify before accepting.'
      );
      return;
    }
    ...
    try {
      const { error: rpcError } = await supabase.rpc('respond_to_mover_booking', {
        p_booking_id: bookingId,
        p_decision: decision,
      ...
      const nextMoverId = data?.mover?.id ?? null;

      setDecision(null);
      setReason('');
      await loadBooking(true);

      if (decision === 'confirm' || decision === 'not_sure') {
        if (nextMoverId) navigate('chat', nextMoverId);
        else navigate('dashboard');
      } else {
        navigate('dashboard');
      }
    } catch (err) {
      console.error('Mover booking response failed:', err);
      setActionError(getRpcErrorMessage(err, 'Unable to update this booking.'));
    } finally {
        ...
        <div className="space-y-6">
          <section className="card p-6 sm:p-7">
            ...
            <div className="relative">
              <div className="absolute left-[11px] top-8 bottom-8 w-px bg-gray-200 dark:bg-brand-700" />
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-xl bg-gray-50 p-4 dark:bg-brand-800/30">
                ...
                <p className="mt-1 font-semibold text-gray-900 dark:text-white">
                  {booking.distance_km !== null ? `${booking.distance_km.toFixed(1)} km` : '—'}
                </p>
              </div>
              <div className="rounded-xl bg-gray-50 p-4 dark:bg-brand-800/30">
                ...
                <div>
                  ...
                  {(renter?.city || renter?.county) && (
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {[renter.city, renter.county].filter(Boolean).join(', ')}
                    </p>
                  )}
                </div>
              </div>

              {renter?.phone && booking.status === 'confirmed' && (
                <a
                  href={`tel:${renter.phone}`}
                  className="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-sm font-medium text-brand-600 hover:bg-gray-50 dark:border-brand-800 dark:text-brand-400 dark:hover:bg-brand-800/40"
                >
            ...
            {booking.status !== 'confirmed' && renter?.phone && (
              <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-brand-800 dark:bg-brand-800/30">
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  Renter contact details become available after you accept the booking.
                </p>
              </div>
            )}
          </section>
          ...
          <section className="card p-6 sm:p-7">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-xl bg-gray-50 p-4 dark:bg-brand-800/30">
                <p className="text-xs text-gray-500 dark:text-gray-400">Number plate</p>
                <p className="mt-1 font-semibold text-gray-900 dark:text-white">
                  {mover?.number_plate ?? '—'}
                </p>
              </div>
              <div className="rounded-xl bg-gray-50 p-4 dark:bg-brand-800/30">
                <p className="text-xs text-gray-500 dark:text-gray-400">Operating area</p>
                <p className="mt-1 font-semibold text-gray-900 dark:text-white">
                  {[mover?.operating_city, mover?.operating_county].filter(Boolean).join(', ') || '—'}
                </p>
              </div>
            </div>
          </section>
          ...
          {booking.status === 'confirmed' && (
            <section className="card p-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="font-bold text-gray-900 dark:text-white">Booking accepted</h2>
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Continue the conversation with the renter to finalize the moving schedule.
                  </p>
                  <button
                    type="button"
                    onClick={() => mover?.id && navigate('chat', mover.id)}
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-brand-950">
            <div className="flex items-start gap-3">
              {decision === 'confirm' ? (
                ...
                <MessageCircle className="mt-0.5 h-6 w-6 shrink-0 text-brand-600 dark:text-brand-400" />
              ) : (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  {decision === 'confirm'
                    ...
                    : decision === 'not_sure'
                      ? 'Your message will be sent to the renter so you can clarify the request before confirming.'
