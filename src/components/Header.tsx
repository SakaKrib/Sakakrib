...
export default function Header() {
  ...
  const { navigate, view, setAuthModalOpen } = useNav();
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/95 backdrop-blur-md dark:border-brand-800 dark:bg-brand-950/95">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {mobileMenuOpen && <div className="border-t border-gray-200 py-3 dark:border-brand-800 md:hidden"><nav className="flex flex-col gap-1">{navItems.map((item) => <button key={item.view} onClick={() => { navigate(item.view); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-gray-300 px-4 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><item.icon className="h-4 w-4" />{item.label}</button>)}<p className="px-4 pt-3 text-xs font-semibold uppercase tracking-wide text-gray-400">Professionals</p>{professionalItems.map((item) => <button key={item.view} onClick={() => goTo(item.view)} className="flex items-center gap-3 rounded-full border border-gray-300 px-4 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><item.icon className="h-4 w-4" />{item.label}</button>)}{profile ? <><button onClick={() => { navigate('profile'); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-gray-300 px-4 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><User className="h-4 w-4" /> Profile</button><button onClick={() => { navigate('dashboard'); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-gray-300 px-4 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><LayoutDashboard className="h-4 w-4" /> Dashboard</button><button onClick={() => { signOut(); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-primary-300 px-4 py-2.5 text-left text-sm font-medium text-primary-600"><LogOut className="h-4 w-4" /> Sign Out</button></> : <button onClick={() => { setAuthModalOpen(true); setMobileMenuOpen(false); }} className="btn-primary mt-2">Sign In / Get Started</button>}</nav></div>}
      </div>
    </header>
  );
}
