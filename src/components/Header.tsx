      ...
      <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden items-center gap-1 md:flex">
            {navItems.map((item) => <button key={item.view} onClick={() => navigate(item.view)} className={cn('rounded-full border px-2 py-2 text-sm font-medium transition-colors', view === item.view ? 'border-btnblue-500 bg-btnblue-50 text-btnblue-700 dark:border-btnblue-400 dark:bg-btnblue-900/30 dark:text-btnblue-300' : 'border-gray-300 text-gray-600 hover:border-btnblue-400 hover:text-btnblue-600 dark:border-brand-700 dark:text-gray-300 dark:hover:border-btnblue-500 dark:hover:text-btnblue-400')}>{item.label}</button>)}
            {profile?.role === 'renter' && !isAdmin && ( <><div className="relative">
              <button onClick={() => setProfessionalsOpen((open) => !open)} className="flex items-center gap-1 rounded-full border border-gray-300 px-2 py-2 text-sm font-medium text-gray-600 hover:border-btnblue-400 hover:text-btnblue-600 dark:border-brand-700 dark:text-gray-300"><Building2 className="h-4 w-4" /> Professionals <ChevronDown className="h-4 w-4" /></button>
        ...
        {mobileMenuOpen && <div className="border-t border-gray-200 py-3 dark:border-brand-800 md:hidden"><nav className="flex flex-col gap-1">{navItems.map((item) => <button key={item.view} onClick={() => { navigate(item.view); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-gray-300 px-2 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><item.icon className="h-4 w-4" />{item.label}</button>)} {profile?.role === 'renter' && !isAdmin && ( <><p className="px-2 pt-3 text-xs font-semibold uppercase tracking-wide text-gray-400">Professionals</p>{professionalItems.map((item) => (<button key={item.view} onClick={() => goTo(item.view)} className="flex items-center gap-3 rounded-full border border-gray-300 px-2 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><item.icon className="h-4 w-4" />{item.label}</button>))} </>)}</nav><nav className="flex w-full min-w-0 flex-wrap gap-2 mt-10">{profile ? <><button onClick={() => { navigate('profile'); setMobileMenuOpen(false); }} className="flex min-w-0 flex-1 items-center gap-3 rounded-full border border-gray-300 px-2 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><User className="h-4 w-4" /> Profile</button><button onClick={() => { navigate('dashboard'); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-gray-300 px-2 py-2.5 text-left text-sm font-medium text-gray-600 dark:border-brand-700 dark:text-gray-300"><LayoutDashboard className="h-4 w-4" /> Dashboard</button><button onClick={() => { signOut(); setMobileMenuOpen(false); }} className="flex items-center gap-3 rounded-full border border-primary-300 px-2 py-2.5 text-left text-sm font-medium text-primary-600"><LogOut className="h-4 w-4" /> Sign Out</button></> : <button onClick={() => { setAuthModalOpen(true); setMobileMenuOpen(false); }} className="btn-primary mt-2">Sign In / Get Started</button>}</nav></div>}
      </div>
    </header>
  );
}
