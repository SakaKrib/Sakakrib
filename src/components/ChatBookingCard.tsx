...
interface Props { message?: ChatMessage | null; booking?: Booking | null; isMover: boolean; responding: boolean; onRespond: (decision: 'confirm' | 'not_sure' | 'cancel') => void; onCancel: () => void; onSchedule: () => void; }
const normalize = (value: unknown) => String(value ?? '').trim().toLowerCase().replace(/-/g, '_');
const displayDate = (value: string | null | undefined) => value ? new Date(`${value}T00:00:00+03:00`).toLocaleDateString('en-KE', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }) : '—';
...
export default function ChatBookingCard({ message, booking, isMover, responding, onRespond, onCancel, onSchedule }: Props) {
  ...
  const dropoffLng = event.dropoff_longitude;
  const hasPickupCoordinates = typeof pickupLat === 'number' && typeof pickupLng === 'number';
  const hasDropoffCoordinates = typeof dropoffLat === 'number' && typeof dropoffLng === 'number';
  const isRequest = message?.message_type === 'booking_request';
  const isResponse = message?.message_type === 'booking_response';
  const canMoverRespond = isMover && isRequest && status === 'pending';
  const isRenterPending = !isMover && status === 'pending';
  const isConfirmed = status === 'confirmed' || decision === 'confirm';
  const isDeclined = status === 'cancelled' || decision === 'cancel' || decision === 'not_sure';
  return <article className="w-full max-w-xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-brand-800 dark:bg-brand-950">
    <div className="border-b border-gray-100 bg-gray-50/80 px-4 py-3 dark:border-brand-800 dark:bg-brand-900/60"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-100 text-brand-700 dark:bg-brand-900 dark:text-brand-300"><MessageCircle className="h-4 w-4" /></span><div><p className="text-xs font-semibold uppercase tracking-wide text-brand-600 dark:text-brand-400">Moving request</p><p className="text-sm font-semibold text-gray-900 dark:text-white">{isResponse ? 'Mover response' : 'Request details'}</p></div></div><span className={cn('rounded-full px-2.5 py-1 text-[10px] font-bold uppercase', isConfirmed ? 'bg-success-50 text-success-700 dark:bg-success-900/20 dark:text-success-300' : isDeclined ? 'bg-error-50 text-error-700 dark:bg-error-900/20 dark:text-error-300' : 'bg-warning-50 text-warning-700 dark:bg-warning-900/20 dark:text-warning-300')}>{status || 'pending'}</span></div></div>
