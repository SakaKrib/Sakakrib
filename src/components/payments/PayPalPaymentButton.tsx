...
interface SubscriptionApproveResponse { success?: boolean; detail?: string; message?: string; status?: string; }
...
export default function PayPalPaymentButton({ mode, paymentIntentId, planId, billingCycle, listingId, onPreparePayPalPayment, onSuccess, onError, disabled = false }: PayPalPaymentButtonProps) {
  ...
  const fail = (message: string) => { setWorking(false); setError(message); onError?.(message); };
  ...
  const approveOneTime = async ({ orderId }: OnApproveDataOneTimePayments) => {
    ...
    try {
      ...
      setWorking(false); setError(null); await onSuccess();
  ...
  const approveSubscription = async ({ subscriptionId }: OnApproveDataSubscriptions) => {
    ...
    try {
      ...
      subscriptionInvoiceIdRef.current = null; setWorking(false); setError(null); await onSuccess();
    } catch (error) { fail(error instanceof Error ? error.message : 'Unable to confirm PayPal subscription.'); }
  };
  return <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-900"><PayPalProvider clientId={clientId} environment={environment} components={mode === 'SUBSCRIPTION' ? ['paypal-subscriptions'] : ['paypal-payments']} pageType="checkout">{mode === 'ONE_TIME' ? <PayPalOneTimePaymentButton createOrder={createOneTimeOrder} onApprove={approveOneTime} onCancel={() => setWorking(false)} onError={(sdkError) => fail(sdkError.message || 'PayPal payment failed.')} presentationMode="auto" disabled={disabled || working} type="pay" /> : <PayPalSubscriptionButton createSubscription={createSubscription} onApprove={approveSubscription} onCancel={() => setWorking(false)} onError={(sdkError) => fail(sdkError.message || 'PayPal subscription checkout failed.')} presentationMode="auto" disabled={disabled || working} type="subscribe" />}</PayPalProvider>{error && <div className="mt-3 rounded-lg bg-error-50 px-3 py-2 text-sm text-error-700 dark:bg-error-900/20 dark:text-error-400" role="alert">{error}</div>}{working && <p className="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">Confirming your payment securely…</p>}</div>;
}
