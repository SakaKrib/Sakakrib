...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const applyGatewaySession = (result: GatewaySession): boolean => {
    ...
    if (result.requiresEmailVerification) {
      requireEmailVerification(
        result.email ?? result.profile?.email ?? result.user?.email ?? null,
      );
    }
    return false;
  };
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        console.error('HttpOnly auth initialization error:', error);
        if (mounted) {
          clearAuthState();
          clearVerificationState();
        }
    ...
    const refreshWhenVisible = async () => {
      ...
      try {
        ...
        console.error('HttpOnly auth visibility refresh error:', error);
      }
    };
  ...
  const signUp = async (
    ...
    try {
      ...
      if (result.sessionCreated) await loadSession();
  ...
  const signIn = async (email: string, password: string): Promise<AuthResult> => {
    ...
    try {
      ...
      if (!applyGatewaySession(result)) {
        ...
        return {
          error: resend.error
            ? `Your email is not verified. ${resend.error}`
      ...
      if (/not confirmed|not verified|email.*confirm/i.test(message)) {
        ...
        return {
          error: resend.error
            ? `Your email has not been verified. ${resend.error}`
            : 'Your email is not verified. A new verification code has been sent to your email.',
          requiresEmailVerification: true,
        };
      }
  ...
  const signInWithGoogle = async (): Promise<void> => {
    // Supabase performs the OAuth authorization, but the callback is handled
    // by auth-gateway, which exchanges the code and writes HttpOnly cookies.
    const redirectTo = authGateway.oauthCallbackUrl(window.location.origin);
  ...
  const signOut = async (): Promise<void> => {
    try {
      ...
      console.error('HttpOnly sign-out error:', error);
    } finally {
      clearAuthState();
      clearVerificationState();
    }
  ...
  const setRole = async (role: UserRole): Promise<{ error: string | null }> => {
    ...
    try {
      await authGateway.setRole(role);
      await loadSession();
      return { error: null };
    } catch (error) {
      return { error: error instanceof Error ? error.message : 'Unable to set role.' };
    }
  };
