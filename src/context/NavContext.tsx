...
const VALID_APP_VIEWS: AppView[] = [
  'home', 'listings', 'listing-detail', 'listing-manage', 'movers',
  'mover-detail', 'mover-booking-detail', 'chat', 'community',
  'post-listing', 'register-mover', 'register-landlord', 'kyc-verify',
  'dashboard', 'my-bookings', 'my-listings', 'profile',
  'subscription-plans', 'pms-dashboard', 'renter-invoices',
  'renter-payment', 'mover-tracking', 'renter-calendar', 'notifications',
  'admin-user-details',
];
...
export function NavProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      switch (nextView) {
        case 'listing-detail': setSelectedListingId(hashId ?? null); break;
        case 'listing-manage': setSelectedListingManageId(hashId ?? null); break;
        case 'mover-detail': setSelectedMoverId(hashId ?? null); break;
        case 'mover-booking-detail': setSelectedMoverBookingId(hashId ?? null); break;
        case 'chat': setSelectedChatMoverId(hashId ?? null); break;
        case 'admin-user-details': setSelectedAdminUserId(hashId ?? null); break;
        default: break;
      }
    };
  ...
  const navigate = (newView: AppView, id?: string) => {
    ...
    switch (newView) {
      case 'listing-detail': if (id) setSelectedListingId(id); break;
      case 'listing-manage': if (id) setSelectedListingManageId(id); break;
      case 'mover-detail': if (id) setSelectedMoverId(id); break;
      case 'mover-booking-detail': if (id) setSelectedMoverBookingId(id); break;
      case 'chat': if (id) setSelectedChatMoverId(id); break;
      case 'admin-user-details': if (id) setSelectedAdminUserId(id); break;
      default: break;
    }
