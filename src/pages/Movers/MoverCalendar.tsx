...
export default function MoverCalendar({ schedule, onOpen }: Props) {
  ...
  const eventsForDate = useMemo(
    () =>
      schedule.filter(
        (event) =>
          ['TENTATIVE', 'CONFIRMED'].includes(eventStatus(event.status)) &&
          dayKey(new Date(event.starts_at)) === dayKey(selectedDate),
      ),
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            View scheduled moving jobs by date.
          </p>
        </div>
              ...
              return (
                <div className="flex items-center justify-between px-2 py-1">
                  <button
                    ...
                    className="rounded-md p-1 hover:bg-brand-200 disabled:opacity-40"
                  >
          ...
          <div className="grid grid-cols-7 gap-2">
            {monthDays.map((date, index) => {
              ...
              const events = schedule.filter(
                (event) =>
                  ['TENTATIVE', 'CONFIRMED'].includes(eventStatus(event.status)) &&
                  dayKey(new Date(event.starts_at)) === key,
              );
                ...
                <button
                  ...
                  className={`min-h-16 rounded-xl border p-2 text-left transition ${
                    selected
                      ...
                      : 'border-gray-200 bg-white hover:border-brand-300 dark:border-brand-800 dark:bg-brand-900'
                  }`}
                >
                  ...
                  {events.length > 0 && (
                    <span className="mt-1 block text-xs font-semibold text-brand-600 dark:text-brand-400">
          ...
          <div className="mt-5 rounded-xl border border-gray-200 p-4 dark:border-brand-800">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Scheduled jobs</p>
              </div>
              <button
                type="button"
            ...
            {eventsForDate.length === 0 ? (
              <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">
                No scheduled moving jobs for this date.
              </p>
            ) : (
              <div className="mt-4 space-y-2">
                {eventsForDate.map((event) => (
                  <button
                    key={event.id}
                    type="button"
                    onClick={() => onOpen(event.booking_id)}
                    className="w-full rounded-xl bg-brand-50 p-3 text-left hover:bg-brand-100 dark:bg-brand-900/30 dark:hover:bg-brand-900/50"
                  >
                    <p className="text-sm font-semibold text-brand-800 dark:text-brand-200">
                      {new Date(event.starts_at).toLocaleTimeString('en-KE', {
                        hour: 'numeric',
                        minute: '2-digit',
                      })}{' '}
                      · {event.title || 'Moving job'}
                    </p>
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      Ends{' '}
                      {new Date(event.ends_at).toLocaleTimeString('en-KE', {
                        hour: 'numeric',
                        minute: '2-digit',
                      })}
                    </p>
