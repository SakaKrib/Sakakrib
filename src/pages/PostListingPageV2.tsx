...
import PMSPlanSelector, { type PMSBillingCycle, type PMSPlanName } from '@/components/PMS/PMSPlanSelector';
...
type UnitAvailability = 'available' | 'occupied' | 'reserved';
interface MediaItem { file?: File; url: string; label: string; type: 'photo' | 'video'; id?: string; }
interface PropertyUnit { id: string; unitNumber: string; unitType: string; rent: string; depositAmount: string; size: string; beds: string; baths: string; availability: UnitAvailability; description: string; photos: MediaItem[]; }
...
export default function PostListingPageV2() {
  ...
  const [pmsSelectorOpen, setPmsSelectorOpen] = useState(false);
  const [pmsPaymentCompleted, setPmsPaymentCompleted] = useState(false);
  ...
  }, [selectedPostListingDraftId, draftId]);

  useEffect(() => {
    let cancelled = false;
    if (!profile?.id || !listingRole) return;
    setEntitlementLoading(true);
    fetchListingEntitlement(listingRole, profile.id)
      .then((entitlement) => {
        if (cancelled) return;
  ...
  const isPropertyManagementListing = pmsMode || subscriptionStatus === 'active' || subscriptionStatus === 'trial';
  const paymentRequired = !isPropertyManagementListing && Boolean(listingEntitlement?.requiresIndividualPayment);
  const listingPaymentRequirement: ListingPaymentRequirement = paymentRequired ? 'required' : 'not_required';
  ...
  const STEPS = paymentRequired ? [...BASE_STEPS, 'Payment'] : BASE_STEPS;
  ...
  const buildListingPayload = (): ListingFormPayload => ({
    ...
    deposit_structure: depositStructure || null,
  ...
  const draftPayload = () => ({
    ...buildListingPayload(),
    draft_ui: { customCity, customSize, units, termsAccepted, reviewConfirmed, step, pmsMode: isPropertyManagementListing },
  });

  const saveDraftNow = async (): Promise<string> => {
    ...
    if (!id) throw new Error('The listing draft could not be saved.');
    setDraftId(id);
    if (!draftId) navigate('post-listing', id);
    return id;
  };

  const ensureDraft = async () => draftId || saveDraftNow();

  useEffect(() => {
    let cancelled = false;
    const loadDraft = async () => {
      if (!selectedPostListingDraftId || !profile?.id || !listingRole) {
      ...
      try {
        ...
        if (cancelled) return;
        const d = draft.data || {};
        const id = String(draft.listing_id || draft.id);
        setDraftId(id);
        setTitle(String(d.title || ''));
        setDescription(String(d.description || ''));
        setCity(String(d.city || ''));
        setCounty(String(d.county || ''));
        setLocationSearch(String(d.location_search || ''));
        setLatitude(d.latitude == null ? null : Number(d.latitude));
        setLongitude(d.longitude == null ? null : Number(d.longitude));
        setPropertyName(String(d.property_name || ''));
        setPropertyType(String(d.property_type || ''));
        setPrice(d.price_kes == null ? '' : String(d.price_kes));
        setListingType(d.listing_type === 'sale' ? 'sale' : 'rent');
        setDepositRequired(Boolean(d.deposit_required));
        setDepositStructure(d.deposit_structure === 'installments' ? 'installments' : 'fixed');
        setDepositAmount(d.deposit_amount == null ? '' : String(d.deposit_amount));
        setSize(String(d.size || ''));
        setBeds(d.beds == null ? '1' : String(d.beds));
        setBaths(d.baths == null ? '1' : String(d.baths));
        setPhone(String(d.contact_phone || profile.phone || ''));
        setEmail(String(d.contact_email || profile.email || ''));
        setSocialLinks(Array.isArray(d.social_links) ? d.social_links as SocialLink[] : []);
        setBookingEnabled(Boolean(d.booking_enabled));
        setPaymentEnabled(Boolean(d.payment_enabled));
        setPmsMode(Boolean(d.is_property_management));
        const ui = d.draft_ui as Record<string, unknown> | undefined;
        if (ui) {
          setCustomCity(String(ui.customCity || ''));
          setCustomSize(String(ui.customSize || ''));
          setTermsAccepted(Boolean(ui.termsAccepted));
          setReviewConfirmed(Boolean(ui.reviewConfirmed));
          if (Array.isArray(ui.units)) setUnits(ui.units as PropertyUnit[]);
          if (typeof ui.step === 'number') setStep(Math.max(0, ui.step));
          if (typeof ui.pmsMode === 'boolean') setPmsMode(Boolean(ui.pmsMode));
        }
        const media = await protectedGet<ExistingMedia[]>(`/api/listings/media/?listing_id=${encodeURIComponent(id)}`);
        if (cancelled) return;
        ...
        const savedVideo = (media || []).find((m) => m.media_type === 'video' && !m.unit_id);
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Unable to load the saved listing draft.');
          navigate('post-listing');
        }
      } finally {
        if (!cancelled) { draftHydrated.current = true; setDraftLoading(false); }
      }
    };
    loadDraft();
    return () => { cancelled = true; };
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try { await saveDraftNow(); } catch (err) { console.warn('Draft auto-save failed:', err); }
    }, 800);
    return () => { if (saveTimer.current) clearTimeout(saveTimer.current); };
  }, [profile?.id, listingRole, draftId, draftLoading, success, step, pmsMode, propertyName, propertyType, units, bookingEnabled, paymentEnabled, reviewConfirmed, city, customCity, county, latitude, longitude, locationSearch, price, listingType, depositRequired, depositStructure, depositAmount, phone, email, socialLinks, photos, video, title, description, size, customSize, beds, baths]);
  ...
  const handleUnitPhotoUpload = async (unit: PropertyUnit, files: FileList | null) => {
    ...
    const remaining = Math.max(0, MAX_PHOTOS - unit.photos.length);
    if (!remaining) { setError('You can upload a maximum of 7 photos per unit.'); return; }
    try {
      const id = await ensureDraft();
      const selected = Array.from(files).filter((file) => file.type.startsWith('image/') && file.size <= MAX_PHOTO_SIZE).slice(0, remaining);
      ...
      for (let index = 0; index < selected.length; index += 1) {
        ...
        formData.append('label', `Unit Photo ${unit.photos.length + index + 1}`);
        formData.append('position', String(unit.photos.length + index));
        await protectedUpload<ExistingMedia>('/api/listings/media/', formData);
      }
      ...
      updateUnit(unit.id, 'photos', unitMedia);
      setError(null);
  ...
  const canProceed = () => {
    if (isPropertyManagementListing) {
      switch (step) {
        ...
        case 1: return units.length > 0 && units.every((u) => Number(u.rent) > 0 && u.unitNumber.trim() !== '' && u.unitType.trim() !== '' && Number.isInteger(Number(u.beds)) && Number(u.beds) >= 0 && Number.isInteger(Number(u.baths)) && Number(u.baths) >= 0 && u.photos.length >= 3 && u.photos.length <= 7);
        case 2: return units.length > 0 && units.every((u) => Number(u.rent) > 0);
        ...
        default: return step === paymentStepIndex ? paymentCompleted : false;
      }
    }
    switch (step) {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        ...
        if (detectedCity) {
          ...
          if (matchingCity) setCity(matchingCity); else { setCustomCity(detectedCity); setCity('custom'); }
        }
        if (detectedCounty) {
          const normalizedCounty = detectedCounty.replace(/ County$/i, '').trim();
          setCounty(KENYAN_COUNTIES.find((item) => item.toLowerCase() === normalizedCounty.toLowerCase()) || normalizedCounty);
        }
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=6&countrycodes=ke&q=${encodeURIComponent(query)}`, { headers: { Accept: 'application/json' } });
        if (!response.ok) throw new Error(`Location search failed: ${response.status}`);
        const data = await response.json();
        setLocationSuggestions(data.map((item: any) => ({ display_name: item.display_name, lat: item.lat, lon: item.lon, place_id: item.place_id, type: item.type, address: item.address })));
  ...
  const handlePhotoUpload = async (files: FileList | null) => {
    ...
    try {
      const id = await ensureDraft();
      const validFiles = Array.from(files).slice(0, remaining).filter((file) => file.type.startsWith('image/') && file.size <= MAX_PHOTO_SIZE);
      if (!validFiles.length) { setError('Please select valid image files no larger than 10 MB.'); return; }
      for (const [index, file] of validFiles.entries()) {
        const formData = new FormData(); formData.append('listing_id', id); formData.append('file', file); formData.append('media_type', 'photo'); formData.append('label', `Photo ${photos.length + index + 1}`); formData.append('position', String(photos.length + index));
        await protectedUpload<ExistingMedia>('/api/listings/media/', formData);
      }
      const saved = await protectedGet<ExistingMedia[]>(`/api/listings/media/?listing_id=${encodeURIComponent(id)}`);
      setPhotos((saved || []).filter((m) => m.media_type === 'photo' && !m.unit_id).sort((a, b) => a.position - b.position).map((m) => ({ id: m.id, url: m.url, label: m.label || 'Photo', type: 'photo' })));
      setError(null);
  ...
  const handleVideoUpload = async (file: File | null) => {
    ...
    try {
      const id = await ensureDraft(); const formData = new FormData(); formData.append('listing_id', id); formData.append('file', file); formData.append('media_type', 'video'); formData.append('label', 'Walkthrough Video'); formData.append('position', '0');
      const saved = await protectedUpload<ExistingMedia>('/api/listings/media/', formData);
      setVideo({ id: saved.id, url: saved.url, label: saved.label || 'Walkthrough Video', type: 'video' }); setError(null);
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to upload video.'); }
  };

  const updatePhotoLabel = (index: number, label: string) => setPhotos((current) => current.map((p, i) => i === index ? { ...p, label } : p));
  const removePhoto = async (index: number) => { const photo = photos[index]; setPhotos((current) => current.filter((_, i) => i !== index)); if (photo?.id) { try { await protectedPost(`/api/listings/media/${encodeURIComponent(photo.id)}/delete/`, {}); } catch { /* reconcile on reload */ } } };
  const removeVideo = async () => { const current = video; setVideo(null); if (current?.id) { try { await protectedPost(`/api/listings/media/${encodeURIComponent(current.id)}/delete/`, {}); } catch { /* reconcile on reload */ } } };

  const preparePayPalPayment = async (): Promise<string> => {
    if (paymentIntentId) return paymentIntentId;
    const id = await ensureDraft();
    const intent = await createListingPaymentIntent(buildListingPayload(), id);
  ...
  const handleListingPayment = async (): Promise<boolean> => {
    ...
    if (selectedPaymentMethod === 'PAYPAL') { setError('Use the PayPal button to complete the payment.'); return false; }
    ...
    try {
      ...
      const response = await protectedPost<{ success?: boolean; message?: string; error?: string }>('/api/payments/listing/start/', { payment_intent_id: intentId, provider: 'mpesa', phone_number: phone.trim() });
      if (!response?.success) throw new Error(response?.error || response?.message || 'The payment request could not be started.');
      if (!await waitForListingPaymentIntent(intentId)) throw new Error('The listing payment was not confirmed. If you completed the M-Pesa prompt, please wait a moment and try again.');
  ...
  const finalizeNormalListing = async () => {
    const id = await ensureDraft();
    if (paymentRequired && (!paymentCompleted || !paymentIntentId)) throw new Error('Please complete the listing payment before publishing.');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isPropertyManagementListing) {
        if (subscriptionStatus === 'active' || subscriptionStatus === 'trial' || pmsPaymentCompleted) {
          await finalizeNormalListing();
          return;
        }
        const id = await saveDraftNow();
        if (!id) throw new Error('Unable to save the property-management draft.');
        setPmsSelectorOpen(true);
        return;
      }
      await finalizeNormalListing();
  ...
  const handleContinueToSubscription = async () => {
    try {
      const id = await saveDraftNow();
      if (!id) throw new Error('Unable to save this listing before subscription checkout.');
      setPmsMode(true);
      setPmsPaymentCompleted(false);
      setPaymentCompleted(false);
      setPaymentIntentId(null);
      setPaymentModalOpen(false);
      setStep(0);
      setError(null);
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to switch this listing to property management.'); }
  };

  const handlePmsPaymentSuccess = async () => {
    setPmsSelectorOpen(false);
    setPmsPaymentCompleted(true);
    try {
      const id = draftId || await ensureDraft();
      try { const response = await protectedPost<{ caption?: string }>(`/api/listings/${encodeURIComponent(id)}/ai-caption/`, {}); if (response?.caption) setAiCaption(response.caption.trim()); } catch (aiError) { console.warn('AI caption generation failed:', aiError); }
      setSuccess(true);
    } catch (err) { setError(err instanceof Error ? err.message : 'Subscription payment succeeded, but the listing could not be refreshed.'); }
  };

  const removePmsMode = () => {
    if (subscriptionStatus === 'active' || subscriptionStatus === 'trial') return;
    setPmsMode(false); setPmsSelectorOpen(false); setStep(0); setPmsPaymentCompleted(false); setError(null);
  };
  ...
  return <>
    {pmsMode && subscriptionStatus !== 'active' && subscriptionStatus !== 'trial' && <div className="mx-auto w-full max-w-5xl px-2 pt-6 sm:px-6 lg:px-8"><div className="flex items-center justify-between rounded-xl border border-brand-200 bg-brand-50 px-4 py-3 dark:border-brand-700 dark:bg-brand-900/30"><div><p className="text-sm font-semibold text-brand-800 dark:text-brand-200">Property Management listing selected</p><p className="text-xs text-brand-700 dark:text-brand-300">Complete the property, units, media and review steps, then choose your subscription.</p></div><button type="button" onClick={removePmsMode} className="inline-flex items-center gap-1 rounded-lg border border-brand-300 px-3 py-2 text-xs font-semibold text-brand-800 dark:border-brand-600 dark:text-brand-200"><X className="h-4 w-4" />Use standard listing</button></div></div>}
    <PropertyListingForm
    ...
    />
    <ListingPaymentModal open={paymentModalOpen} onClose={() => setPaymentModalOpen(false)} role={listingRole} amountKes={LISTING_FEE_KES} paymentLoading={paymentLoading} paymentCompleted={paymentCompleted} selectedPaymentMethod={selectedPaymentMethod} onSelectPaymentMethod={setSelectedPaymentMethod} onPayNow={handleListingPayment} subscriptionPlans={subscriptionPlans} subscriptionPlansLoading={subscriptionPlansLoading} subscriptionPlansError={subscriptionPlansError} onContinueToSubscription={handleContinueToSubscription} paypalAvailable={Boolean(import.meta.env.VITE_PAYPAL_CLIENT_ID)} paymentIntentId={paymentIntentId} onPreparePayPalPayment={preparePayPalPayment} onPayPalSuccess={() => { setPaymentCompleted(true); setSelectedPaymentMethod('PAYPAL'); setPaymentModalOpen(false); setError(null); }} error={error} />
    {pmsSelectorOpen && <div className="fixed inset-0 z-[70] overflow-y-auto bg-black/50 px-3 py-8"><div className="mx-auto max-w-6xl rounded-2xl bg-white p-4 shadow-2xl dark:bg-gray-950 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-bold text-gray-900 dark:text-white">Choose your property-management subscription</h2><p className="text-sm text-gray-500">Your current listing draft and units will be linked to this subscription.</p></div><button type="button" onClick={() => setPmsSelectorOpen(false)} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"><X className="h-5 w-5" /></button></div><PMSPlanSelector role={listingRole === 'landlord' ? 'landlord' : 'real_estate'} listingId={draftId} billingCycle={'MONTHLY' as PMSBillingCycle} selectedPlanName={null as PMSPlanName | null} onPlanChange={() => undefined} onBillingCycleChange={() => undefined} onPaymentSuccess={handlePmsPaymentSuccess} onGoToDashboard={() => navigate('pms-dashboard')} /></div></div>}
  </>;
}
