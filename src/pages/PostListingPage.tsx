...
export default function PostListingPage() {
  ...
  useEffect(() => {
    if (paymentRequired && step === paymentStepIndex && !paymentCompleted) setPaymentModalOpen(true);
  ...
  const handleContinueToSubscription = (plan: { id: string; name: string }, billingCycle: 'monthly' | 'annual') => {
    try { sessionStorage.setItem('pendingSubscriptionSelection', JSON.stringify({ planId: plan.id, planName: plan.name, billingCycle })); } catch { /* non-fatal */ }
    setPaymentModalOpen(false);
  ...
  const handleUseCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (geoError) => {
        console.error('Geolocation error:', geoError); setUsingGPS(false);
  ...
  const buildListingPayload = (): ListingFormPayload => ({
    ...
    listing_type: listingType?.trim() || 'rent', deposit_required: Boolean(depositRequired), deposit_structure: depositStructure?.trim() || null,
    deposit_amount: depositAmount !== '' ? Number(depositAmount) : 0, size: finalSize?.trim() || null, beds: beds !== '' ? Number(beds) : 0,
    baths: baths !== '' ? Number(baths) : 0, contact_phone: phone.trim(), contact_email: email.trim(),
  ...
  const handleListingPayment = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (!intentId) { const intent = await createListingPaymentIntent(buildListingPayload()); intentId = intent.paymentIntentId; setPaymentIntentId(intentId); }
    ...
    } catch (err) { console.error('❌ Listing payment failed:', err); setError(err instanceof Error ? err.message : 'Unable to process the listing payment.'); setPaymentCompleted(false); return false; }
  ...
  const handleVideoUpload = (file: File | null) => {
    ...
    setVideo({ file, url: URL.createObjectURL(file), label: 'Walkthrough Video', type: 'video' }); setError('');
  };
  ...
  useEffect(() => () => { photos.forEach((photo) => { if (photo.url?.startsWith('blob:')) URL.revokeObjectURL(photo.url); }); if (video?.url?.startsWith('blob:')) URL.revokeObjectURL(video.url); }, []);
  ...
  const handleSubmit = async () => {
    setError(null); setSubmitting(true); setAiCaption(''); let listingId: string | null = null;
    try {
      ...
      const existingMedia = await protectedGet<ExistingMedia[]>(`/api/listings/media/?listing_id=${encodeURIComponent(listingId)}`);
      ...
      try {
        ...
        for (let index = 0; index < localPhotos.length; index++) {
          ...
          const media = await protectedUpload<ExistingMedia>('/api/listings/media/', formData);
        ...
        if (video?.file instanceof File && !existingMedia.find((media) => media.media_type === 'video')) {
          ...
          formData.append('label', video.label?.trim() || 'Walkthrough Video');
          formData.append('position', '0');
          const media = await protectedUpload<ExistingMedia>('/api/listings/media/', formData);
          uploadedMediaIds.push(media.id);
        }
        const savedMedia = await protectedGet<ExistingMedia[]>(`/api/listings/media/?listing_id=${encodeURIComponent(listingId)}`);
        ...
        for (const mediaId of uploadedMediaIds) {
          try { await fetch(`${import.meta.env.VITE_DJANGO_API_URL || ''}/api/listings/media/${encodeURIComponent(mediaId)}/`, { method: 'DELETE', credentials: 'include' }); } catch (cleanupError) { console.warn('Some uploaded media could not be cleaned up:', cleanupError); }
        }
        throw mediaError;
      }

      try {
        const response = await protectedPost<{ caption?: string }>(`/api/listings/${encodeURIComponent(listingId)}/ai-caption/`, {});
        const generatedCaption = typeof response?.caption === 'string' ? response.caption.trim() : '';
        if (generatedCaption) setAiCaption(generatedCaption);
      } catch (aiError) { console.warn('AI caption generation failed. Listing and media remain created:', aiError); }

      setCreatedListingId(listingId); setSuccess(true);
    } catch (err) { console.error('❌ Failed to submit listing:', err); setError(err instanceof Error ? err.message : 'Failed to post listing. Please try again.'); window.scrollTo({ top: 0, behavior: 'smooth' }); }
