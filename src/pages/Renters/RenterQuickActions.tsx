import { ArrowRight, CalendarDays, CreditCard, MapPin, Receipt, Truck } from 'lucide-react';

export type RenterQuickAction = 'invoices' | 'payment' | 'find-mover' | 'track-move' | 'calendar';
...
export default function RenterQuickActions({ hasActiveMove = false, hasRentalHome = false, onAction }: RenterQuickActionsProps) {
  const actions = [
    ...(hasRentalHome ? [
      ...
      { id: 'payment' as const, icon: CreditCard, iconClass: 'text-success-600 dark:text-success-400', title: 'Confirm Rent Payment', description: 'Submit an external transaction ID' },
    ] : []),
  ...
  return <section><div className="mb-3"><h2 className="text-base font-bold text-gray-900 dark:text-white">Quick Actions</h2><p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Manage your renter account</p></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
