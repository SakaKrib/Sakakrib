...
import { protectedGet, protectedPost, protectedUpload } from '@/lib/djangoLegacyApi';
...
function areScheduleValuesEqual(a: Partial<MoverScheduleValue> | undefined, b: Partial<MoverScheduleValue> | undefined) { if (a === b) return true; if (!a || !b) return false; return a.date === b.date && a.startTime === b.startTime && a.endTime === b.endTime; }
...
export default function ChatPage() {
  ...
  const [scheduleValue, setScheduleValue] = useState<Partial<MoverScheduleValue> | undefined>(undefined);
  ...
  const handleScheduleChange = useCallback((next: Partial<MoverScheduleValue> | undefined) => { setScheduleValue(previous => areScheduleValuesEqual(previous, next) ? previous : next); }, []);

  const loadMover = useCallback(async () => {
    if (!selectedChatMoverId) { setMover(null); return; }
    const row = await protectedGet<ChatMover>(`/api/core/movers/${encodeURIComponent(String(selectedChatMoverId))}/`);
    setMover(first(row));
  }, [selectedChatMoverId]);
  ...
  const loadData = useCallback(async () => {
    ...
    const [chatResponse, bookingRows, events] = await Promise.all([
      protectedGet<{ messages?: ChatMessage[] }>(`/api/core/chat/?conversation_id=${encodeURIComponent(String(conversationId))}`),
  ...
  const proposeSchedule = useCallback(async () => {
    if (!profile || !mover || !booking || isMover || !scheduleValue || !scheduleValue.date || !scheduleValue.startTime || !scheduleValue.endTime) return;
    setProposing(true);
    ...
    try {
      ...
      await protectedPost('/api/core/chat/', { receiver_id: mover.user_id, content: 'Moving schedule proposed. Please confirm the requested date and time.', message_type: 'schedule_proposed', event_data: { booking_id: booking.id, starts_at: startsAt, ends_at: endsAt } });
  ...
  if (!mover) return <div className="flex min-h-screen flex-col items-center justify-center gap-3 px-6 text-center"><MessageCircle className="h-8 w-8" /><p className="text-sm text-gray-400">Unable to load this chat.</p><button onClick={() => navigate(-1 as any)} className="rounded-lg px-4 py-2">Go back</button></div>;
