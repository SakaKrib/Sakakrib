...
function getBookingIdFromMessage(
  message: ChatMessage,
) {
  const value =
    message.event_data?.booking_id;
...
function getAttachment(
  ...
  const raw =
    message.event_data?.attachments;
...
/**
 * Prevents the controlled schedule picker from causing
 * a render loop when it emits an equivalent object.
 */
function areScheduleValuesEqual(
  a: MoverScheduleValue | null,
  b: MoverScheduleValue | null,
) {
  if (a === b) {
    return true;
  }
...
function ChatImage({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) {
          setUrl(body.signed_url);
        }
      } catch (err) {
        console.error(
          'Failed to sign chat image:',
          err,
        );
  ...
  if (error || !url) {
    return (
      <div className="rounded-xl bg-brand-800/40 px-2 py-3 text-xs text-gray-400">
        Image unavailable
      </div>
    );
  }
...
export default function ChatPage() {
  ...
  const [mover, setMover] =
    useState<ChatMover | null>(null);

  const [booking, setBooking] =
    useState<MovingBooking | null>(null);
  ...
  const [scheduleValue, setScheduleValue] =
    useState<MoverScheduleValue | null>(
      null,
    );
  ...
  /**
   * Used to prevent stale loadData requests
  ...
  /**
   * Keeps the current booking ID available
   * without making the EventSource effect
   * depend on the entire booking object.
   */
  const bookingIdRef =
  ...
  const receiverId = isMover
    ? booking?.renter_id ?? null
    : mover?.user_id ?? null;
  ...
  const blockedIntervals: MoverBlockedInterval[] =
    useMemo(() => {
   ...
   * IMPORTANT PATCH:
   *
   * Do not blindly pass setScheduleValue
   * to the controlled schedule picker.
   *
   * If the picker emits a fresh object containing
   * the same values, React would otherwise receive
   * another state update on every render.
   */
  ...
  const loadMover =
    useCallback(async () => {
      ...
      const rows =
        await protectedGet<ChatMover[]>(
          `/rest/v1/movers?select=*&id=eq.${encodeURIComponent(
            selectedChatMoverId,
          )}&limit=1`,
        );
  ...
  const loadData =
    useCallback(async () => {
      ...
      const bookingFilter = isMover
        ? `mover_id=eq.${encodeURIComponent(
            mover.id,
          )}`
        : `renter_id=eq.${encodeURIComponent(
            profile.id,
          )}&mover_id=eq.${encodeURIComponent(
            mover.id,
          )}`;
      ...
      const [
        ...
        protectedGet<ScheduleEvent[]>(
          `/rest/v1/mover_schedule_events?select=*&mover_id=eq.${encodeURIComponent(
            mover.id,
          )}&order=starts_at.asc`,
        ),
      ]);
       ...
       * if another refresh/load started afterward.
       */
      if (
        generation !==
        loadGenerationRef.current
  ...
  /**
   * Load booking/messages/schedule after
   * the mover and authenticated profile exist.
   */
  useEffect(() => {
    if (
      !profile ||
   ...
   *
   * The booking ID is read from a ref so that
   * ordinary booking object updates do not cause
   * unnecessary EventSource teardown/recreation.
   */
  useEffect(() => {
    if (
      !profile ||
    ...
    const stream =
      new EventSource(
        `${baseUrl.replace(
          /\/+$/,
          '',
        )}/functions/v1/${STREAM_FUNCTION}?${params.toString()}`,
        {
    ...
    stream.onerror = () => {
      /**
       * Native EventSource automatically
  ...
  /**
   * Cleanup realtime connection when
    ...
    useCallback(
      async (file: File) => {
        ...
        if (
          ...
          setPageError(
            'Picture attachments must be smaller than 8 MB.',
          );
          return;
        }
    ...
    useCallback(async () => {
      ...
      try {
        if (
          ...
          throw new Error(
            'The mover must accept the request before a moving time can be proposed.',
          );
        }
        ...
        await protectedPost(
          '/rest/v1/chat_messages',
          {
            ...
            receiver_id:
              mover.user_id,
            content:
              'Moving schedule proposed. Please confirm the requested date and time.',
            message_type:
              'schedule_proposed',
            event_data: {
              booking_id:
        ...
        setScheduleError(
          errorMessage(
            error,
            'Unable to propose the moving schedule.',
          ),
        );
      } finally {
        setProposing(false);
    ...
    useCallback(async () => {
      ...
      try {
        ...
        await protectedPost(
          '/rest/v1/chat_messages',
          {
            ...
            receiver_id:
              booking.renter_id,
            content:
              'The mover confirmed the moving date and time.',
            message_type:
              'schedule_confirmed',
            event_data: {
              booking_id:
        ...
        setPageError(
          errorMessage(
            error,
            'Unable to confirm the moving schedule.',
          ),
        );
      } finally {
        setResponding(false);
  ...
  if (!mover) {
    return (
      <div className="mx-auto max-w-2xl px-2 py-12 text-center">
        ...
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
          The selected mover could not be
          found.
        </p>
  ...
  return (
    <div className="mx-auto max-w-4xl px-2 py-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      The mover has accepted
                      your request. Select a
                      time within their
                      availability.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="rounded-full p-2 text-gray-400 hover:bg-gray-100 dark:hover:bg-brand-800"
        ...
        isMover && (
          <section className="card mb-4 border-2 border-brand-300 p-4 dark:border-brand-600">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h2 className="font-bold text-gray-900 dark:text-white">
                  Schedule awaiting your
                  confirmation
                </h2>

                <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                  {formatDateTime(
                    latestSchedule.starts_at,
                  )}{' '}
                  –{' '}
                  {formatTime(
                    latestSchedule.ends_at,
                  )}
                </p>
        ...
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="mt-1 max-w-md text-sm text-gray-500 dark:text-gray-400">
              ...
              access instructions, moving
              items, and anything the mover
              needs to know.
            </p>
          </div>
        ) : (
              ...
              (message) => {
                ...
                if (
                  ...
                  return (
                    <div
                      ...
                      className="flex justify-center"
                    >
                      <div className="rounded-full bg-success-50 px-2 py-2 text-xs font-medium text-success-700 dark:bg-success-900/30 dark:text-success-400">
                ...
                return (
                  ...
                  >
                    <div
                      className={cn(
                        'max-w-[82%] rounded-2xl px-2 py-2.5',
                        own
              ...
              <button
                ...
                className={cn(
                  ...
                  showScheduler
                    ...
                    : 'text-gray-400 hover:bg-gray-100 hover:text-brand-600 dark:hover:bg-brand-800',
                )}
