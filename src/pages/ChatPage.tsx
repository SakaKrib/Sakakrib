  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        console.error('Failed to sign chat image:', err);
...
export default function ChatPage() {
  ...
  useEffect(() => { bookingIdRef.current = booking?.id ?? null; }, [booking?.id]);
  ...
  const loadMover = useCallback(async () => {
    ...
    const rows = await protectedGet<ChatMover[]>(`/api/core/movers/${encodeURIComponent(selectedChatMoverId)}/`);
    setMover(first(rows));
  }, [selectedChatMoverId]);
  ...
  const loadData = useCallback(async () => {
    ...
    const [chatResponse, bookingRows, events] = await Promise.all([
      protectedGet<ChatMessage[] | { messages?: ChatMessage[] }>(`/api/core/chat/?conversation_id=${encodeURIComponent(conversationId)}`),
    ...
    const generalMessages = Array.isArray(chatResponse) ? chatResponse : chatResponse.messages ?? [];
  ...
  const sendText = useCallback(async () => {
    ...
    try {
      const message = await protectedPost<ChatMessage>('/api/core/chat/message/', { conversation_id: conversationId, receiver_id: receiverId, content: newMessage.trim(), message_type: 'text', event_data: null });
  ...
  const uploadImage = useCallback(async (file: File) => {
    ...
    try {
      ...
      const message = await protectedPost<ChatMessage>('/api/core/chat/message/', { conversation_id: conversationId, receiver_id: receiverId, content: attachment.name, message_type: 'image', event_data: { attachments: [attachment] } });
      if (message) setMessages(previous => [...previous.filter(item => item.id !== message.id), message].sort((a,b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()));
  ...
  const respondToBooking = useCallback(async (decision: 'confirm' | 'cancel') => {
    if (!profile || !booking || !isMover) return; setResponding(true); setPageError(null);
    try { await protectedPost(`/api/core/bookings/${booking.id}/respond/`, { decision, reason: decision === 'cancel' ? 'Mover declined the request.' : null }); await loadData(); }
    catch (error) { setPageError(errorMessage(error, 'Unable to update the booking request.')); } finally { setResponding(false); }
  }, [profile, booking, isMover, loadData]);

  const proposeSchedule = useCallback(async () => {
    ...
    try {
      ...
      await protectedPost(`/api/core/bookings/${booking.id}/schedule/propose/`, { starts_at: startsAt, ends_at: endsAt });
      await protectedPost('/api/core/chat/message/', { conversation_id: conversationId, receiver_id: mover.user_id, content: 'Moving schedule proposed. Please confirm the requested date and time.', message_type: 'schedule_proposed', event_data: { booking_id: booking.id, starts_at: startsAt, ends_at: endsAt } });
  ...
  }, [profile, mover, booking, isMover, scheduleValue, bookingStatus, conversationId, loadData]);

  const confirmSchedule = useCallback(async () => {
    if (!profile || !booking || !isMover) return; setResponding(true); setPageError(null);
    try { await protectedPost(`/api/core/bookings/${booking.id}/schedule/confirm/`, {}); await protectedPost('/api/core/chat/message/', { conversation_id: conversationId, receiver_id: booking.renter_id, content: 'The mover confirmed the moving date and time.', message_type: 'schedule_confirmed', event_data: { booking_id: booking.id } }); await loadData(); }
    catch (error) { setPageError(errorMessage(error, 'Unable to confirm the moving schedule.')); } finally { setResponding(false); }
  }, [profile, booking, isMover, conversationId, loadData]);
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col">
        <header className="flex items-center gap-3 border-b px-4 py-3">
          <button onClick={() => navigate(-1)} className="rounded-lg p-2 hover:bg-muted" aria-label="Back"><ArrowLeft className="h-5 w-5" /></button>
          <div className="min-w-0 flex-1"><h1 className="truncate font-semibold">{mover.business_name || mover.driver_full_name || 'Mover'}</h1><p className="text-xs text-muted-foreground">{mover.operating_city || mover.operating_county || 'Moving service'}</p></div>
          <button onClick={() => void refresh()} disabled={refreshing} className="rounded-lg p-2 hover:bg-muted" aria-label="Refresh"><RefreshCw className={cn('h-5 w-5', refreshing && 'animate-spin')} /></button>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-4 py-4">
          ...
          <div className="space-y-3">
            {messages.map(message => { const attachment = getAttachment(message); const bookingMessageId = getBookingIdFromMessage(message); return <div key={message.id} className={cn('flex', message.sender_id === profile?.id ? 'justify-end' : 'justify-start')}><div className={cn('max-w-[85%] rounded-2xl px-4 py-3', message.sender_id === profile?.id ? 'bg-primary text-primary-foreground' : 'bg-muted')}>
              ...
              {bookingMessageId && <p className="mt-1 text-[10px] opacity-70">Booking #{bookingMessageId.slice(0, 8)}</p>}
              <p className="mt-1 text-[10px] opacity-60">{formatDateTime(message.created_at)}</p>
            </div></div>; })}
