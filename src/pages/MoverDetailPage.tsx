...
import {
  ...
  Calendar, DollarSign, CheckCircle2, CreditCard, MessageCircle
} from 'lucide-react';
...
export default function MoverDetailPage() {
  ...
  useEffect(() => {
    if (!mover || pickupLocation.latitude === null || pickupLocation.longitude === null || dropoffLocation.latitude === null || dropoffLocation.longitude === null) {
      setQuote(null);
      return;
    }
    let cancelled = false;
    const getQuote = async () => {
      ...
      try {
        ...
        const haversine = Math.min(1, Math.max(0, Math.sin(dLatitude / 2) ** 2 + Math.cos(latitude1) * Math.cos(latitude2) * Math.sin(dLongitude / 2) ** 2));
  ...
  const handleMovingDateChange = (date: Date | null) => setMovingDate(formatMovingDate(date));
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    if (!quote || pickupLocation.latitude === null || pickupLocation.longitude === null || dropoffLocation.latitude === null || dropoffLocation.longitude === null) {
    ...
    try {
      ...
      window.setTimeout(() => navigate('chat', mover.user_id), 450);
    } catch (err) {
