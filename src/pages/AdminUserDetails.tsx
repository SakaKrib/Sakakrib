...
try {
  ...
  /* ==================================================
    2. UPDATE THROUGH CANONICAL DJANGO SERVICE
  ================================================== */

  /*
   * The Django application-status service is the single source
   * of truth for:
   *
   * - application status
   * - role transition
   * - KYC completion
   * - verification status
   * - mover lifecycle where applicable
   * - review metadata
   *
   * Do not duplicate that state machine in the frontend.
   */
  await protectedPatch(
    `/api/accounts/admin/users/${encodeURIComponent(String(userId))}/application-status/`,
    {
...
try {
  ...
  setProperties(
    listingRows.map((listing) => ({
      ...listing,
      media:
        mediaByListingId.get(String(listing.id)) ||
        [],
    }))
  );
} catch (err) {
...
return {
  ...
  reference_contacts:
    Array.isArray(mover.reference_contacts)
...
try {
  ...
  /*
   * If there are no approved/created mover records, admin users may
   * have submitted a mover application instead.
   */
  if (rows.length === 0) {
    try {
      const adminApp =
      ...
      console.debug(
        'Failed to fetch mover_applications fallback',
        appErr
      );
    }
  }
...
try {
  ...
  /*
   * Both application-backed and existing mover records now use
   * the same canonical Django application-status endpoint.
   *
   * The backend owns the complete mover lifecycle:
   * application status, role, KYC, verification, availability,
   * mover approval status, and application review metadata.
   */
  if (!mover.user_id) {
    throw new Error(
  ...
  /*
   * If this row came from mover_applications rather than a Mover
   * record, refresh the serialized application from the admin
   * endpoint and convert it back into the mover-shaped object.
   */
  ...
  if (recipientEmail) {
    ...
    /*
     * The send-email compatibility endpoint remains responsible
...
if (status === 'approved') {
return ( <span className="badge bg-success-50 text-success-700 dark:bg-success-900/30 dark:text-success-400"> <CheckCircle2 className="h-3 w-3" />
Approved </span>
);
}
...
if (loading) {
return ( <div className="mx-auto flex min-h-[500px] max-w-7xl items-center justify-center px-2"> <div className="text-center"> <RefreshCw className="mx-auto h-8 w-8 animate-spin text-brand-600" />
...
if (!user) {
return ( <div className="mx-auto max-w-7xl px-2 py-8"> <button
       type="button"
       onClick={handleBack}
...
return ( <div className="mx-auto max-w-7xl px-2 py-6 sm:px-6 lg:px-8">
