...
function isUnpaidInvoice(status?: string | null) {
  const normalized = status?.trim().toLowerCase();
  return !['paid', 'completed', 'settled', 'cancelled', 'canceled'].includes(normalized ?? '');
}

function isActiveBooking(status?: string | null) {
  const normalized = status?.trim().toLowerCase();
  return ['pending', 'confirmed', 'accepted', 'in_progress', 'in-progress', 'ongoing'].includes(normalized ?? '');
}
...
export default function RenterDashboard() {
  ...
  useEffect(() => {
    ...
    if (profile.role !== 'renter') navigate('home');
  ...
  const loadDashboard = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await renterApi.getDashboard(profile.id);
      setData({
  ...
  useEffect(() => {
    if (!authLoading && profile?.role === 'renter' && profile.id) void loadDashboard();
  }, [authLoading, profile?.role, profile?.id, loadDashboard]);
  ...
  const handleQuickAction = useCallback((action: RenterQuickAction) => {
    switch (action) {
      case 'invoices': navigate('renter-invoices'); break;
  ...
  const handleRentPaymentSubmission = useCallback(async (invoiceId: string, transactionId: string) => {
    await renterApi.submitRentPayment(invoiceId, transactionId);
    await loadDashboard();
  }, [loadDashboard]);

  if (authLoading || loading) {
    return <div className="mx-auto flex min-h-[400px] max-w-7xl items-center justify-center px-2 py-8 sm:px-6 lg:px-8"><div className="text-center"><RefreshCw className="mx-auto h-8 w-8 animate-spin text-brand-500" /><p className="mt-3 text-sm text-gray-500 dark:text-gray-400">Loading your renter dashboard...</p></div></div>;
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-2 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-sm font-medium text-brand-600 dark:text-brand-400">Renter Dashboard</p><h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-white sm:text-3xl">Your rental home at a glance</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage your home, rent, invoices, and moving services from one place.</p></div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => navigate('notifications')} aria-label="Notifications" title="Notifications" className={cn('relative flex h-10 w-10 items-center justify-center rounded-xl border', 'border-gray-200 bg-white text-gray-600 transition-colors hover:bg-gray-50', 'focus:outline-none focus:ring-2 focus:ring-brand-500/30', 'dark:border-brand-800 dark:bg-brand-900/40 dark:text-gray-300 dark:hover:bg-brand-800/60')}><Bell className="h-5 w-5" /></button>
          <button type="button" onClick={() => navigate('chat')} aria-label="Messages" title="Messages" className={cn('relative flex h-10 w-10 items-center justify-center rounded-xl border', 'border-gray-200 bg-white text-gray-600 transition-colors hover:bg-gray-50', 'focus:outline-none focus:ring-2 focus:ring-brand-500/30', 'dark:border-brand-800 dark:bg-brand-900/40 dark:text-gray-300 dark:hover:bg-brand-800/60')}><MessageCircle className="h-5 w-5" /></button>
          <button type="button" onClick={() => void loadDashboard()} disabled={loading} className="btn-secondary inline-flex items-center gap-2 text-sm"><RefreshCw className={cn('h-4 w-4', loading && 'animate-spin')} /><span className="hidden sm:inline">Refresh</span></button>
        </div>
      </div>

      {error && <div className="mb-6 flex items-start justify-between gap-3 rounded-xl bg-error-50 px-2 py-3 text-sm text-error-700 dark:bg-error-900/20 dark:text-error-400"><div className="flex items-start gap-2"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><div><p className="font-semibold">Unable to load dashboard</p><p className="mt-0.5 break-words">{error}</p></div></div><button type="button" onClick={() => setError(null)} className="shrink-0 rounded-lg p-1 hover:bg-error-100 dark:hover:bg-error-900/40" aria-label="Dismiss error">×</button></div>}
