...
export default function RenterDashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      setData({
        ...
        invoices: Array.isArray(response?.invoices)
          ? response.invoices
          : [],
        bookings: Array.isArray(response?.bookings)
          ? response.bookings
          : [],
      });
    } catch (err) {
  ...
  useEffect(() => {
    if (!authLoading && profile?.role === 'renter' && profile.id) {
      void loadDashboard();
    }
  }, [authLoading, profile?.role, profile?.id, loadDashboard]);

  const currentInvoice = useMemo(
    () => data.invoices.find((invoice) => isUnpaidInvoice(invoice.status)) ?? null,
    [data.invoices],
  );

  const activeBooking = useMemo(
    () => data.bookings.find((booking) => isActiveBooking(booking.status)) ?? null,
    [data.bookings],
  );

  const monthlyRent =
    data.association?.rent_amount ?? data.unit?.rent ?? null;
  ...
  const handleQuickAction = useCallback(
    (action: RenterQuickAction) => {
      switch (action) {
        ...
        case 'track-move':
          if (activeBooking) {
            navigate('mover-tracking', activeBooking.id);
          } else {
            navigate('movers');
          }
          break;
    ...
    <div className="mx-auto max-w-7xl px-2 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-white sm:text-3xl">
            Your rental home at a glance
          </h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Manage your home, rent, invoices, and moving services from one place.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              'border-gray-200 bg-white text-gray-600 transition-colors hover:bg-gray-50',
              'focus:outline-none focus:ring-2 focus:ring-brand-500/30',
              'dark:border-brand-800 dark:bg-brand-900/40 dark:text-gray-300 dark:hover:bg-brand-800/60',
            )}
          >
      ...
      <section className="mb-6">
        <RenterHomeCard
          home={{
            property: data.property
              ? {
                  ...
                  cover_image_url: data.property.cover_image_url,
                }
              : null,
            unit: data.unit
              ? data.unit
          ...
          onViewProperty={(propertyId) =>
            navigate('listing-detail', propertyId)
          }
        />
      </section>
      ...
      <section className="mb-6">
        <RenterQuickActions
          ...
          hasActiveMove={Boolean(activeBooking)}
        />
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <RenterRentCard
          ...
          onViewInvoices={() => navigate('renter-invoices')}
        />

        <RenterMovingCard
          booking={activeBooking}
          onTrack={(bookingId) => navigate('mover-tracking', bookingId)}
          onFindMover={() => navigate('movers')}
        />
      </div>
    </div>
  );
