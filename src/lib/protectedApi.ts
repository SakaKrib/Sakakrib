...
const getInIds = (value: string | null): string[] => {
  ...
  return match
    ? match[1].split(',').map((id) => decodeURIComponent(id.trim())).filter(Boolean)
    : [];
};
...
export const protectedApi = async <T = unknown>(path: string, init: RequestInit = {}): Promise<T> => {
  ...
  const response = await fetch(`${getFunctionUrl()}${normalizedPath}`, {
    ...init,
    credentials: 'include',
    headers,
  });
  ...
  if (!response.ok) {
    const errorBody = body as ProtectedApiErrorBody | null;
    const error = new Error(
      errorBody?.error ?? errorBody?.message ?? `Protected API request failed (${response.status}).`,
    ) as ProtectedApiException;
    error.status = response.status;
    error.authenticated = errorBody?.authenticated;
    error.authorized = errorBody?.authorized;
...
export const protectedFunctionPost = async <T = unknown>(functionPath: string, body: unknown): Promise<T> => {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      errorBody?.error ?? errorBody?.message ?? `Protected function request failed (${response.status}).`,
    ) as ProtectedApiException;
    error.status = response.status;
    error.authenticated = errorBody?.authenticated;
    error.authorized = errorBody?.authorized;
...
export const protectedGet = async <T = unknown>(path: string, init: RequestInit = {}) =>
  protectedApi<T>(path, { ...init, method: 'GET' });

export const protectedPost = async <T = unknown>(path: string, body: unknown, init: RequestInit = {}) =>
  protectedApi<T>(path, { ...init, method: 'POST', body: JSON.stringify(body) });

export const protectedPatch = async <T = unknown>(path: string, body: unknown, init: RequestInit = {}) =>
  protectedApi<T>(path, { ...init, method: 'PATCH', body: JSON.stringify(body) });

export const protectedPut = async <T = unknown>(path: string, body: unknown, init: RequestInit = {}) =>
  protectedApi<T>(path, { ...init, method: 'PUT', body: JSON.stringify(body) });

export const protectedDelete = async <T = unknown>(path: string, init: RequestInit = {}) =>
