...
const tryAdminBridge = async <T>(path: string, init: RequestInit): Promise<{ handled: boolean; data?: T }> => {
  ...
  if (resource === '/rest/v1/platform_settings' && method === 'GET') {
    const data = await djangoRequest<T>('/api/core/platform-settings/');
  ...
  if (resource === '/rest/v1/landlord_subscriptions' && method === 'GET') {
    ...
    if (userId) {
      const data = await djangoRequest<any>('/api/subscriptions/me/');
      if (!data.subscription_id) return { handled: true, data: [] as T };
      return { handled: true, data: [{ id: data.subscription_id, landlord_id: userId, plan_id: data.plan_id, billing_cycle: data.billing_cycle, status: data.subscription_status, current_period_start: data.current_period_start, current_period_end: data.current_period_end, grace_period_end: data.grace_period_end, auto_renew: data.auto_renew, plan: data.plan_id ? { id: data.plan_id, name: data.plan_name, max_listings: data.max_listings, max_units_per_listing: data.max_units_per_listing } : null }] as T };
    }
  ...
  if (resource === '/rest/v1/real_estate_subscriptions' && method === 'GET') {
    ...
    if (userId) {
      const data = await djangoRequest<any>('/api/subscriptions/me/');
      if (!data.subscription_id) return { handled: true, data: [] as T };
      return { handled: true, data: [{ id: data.subscription_id, real_estate_id: userId, plan_id: data.plan_id, billing_cycle: data.billing_cycle, status: data.subscription_status, current_period_start: data.current_period_start, current_period_end: data.current_period_end, grace_period_end: data.grace_period_end, auto_renew: data.auto_renew, plan: data.plan_id ? { id: data.plan_id, name: data.plan_name, max_listings: data.max_listings, max_units_per_listing: data.max_units_per_listing } : null }] as T };
    }
  ...
  if (resource === '/rest/v1/listing_media' && method === 'GET') {
    ...
    if (listingIds.length) { const rows: unknown[] = []; for (const listingId of listingIds) rows.push(...await djangoRequest<unknown[]>(`/api/listings/media/?listing_id=${encodeURIComponent(listingId)}`)); return { handled: true, data: rows as T }; }
  }

  if (resource === '/rest/v1/movers') {
    if (method === 'GET') {
      const userId = getEqId(query.get('user_id'));
      if (userId && isAdminDetailQuery(query)) { const detail = await adminUser<{ movers: T }>(userId); return { handled: true, data: detail.movers }; }
      const moverId = getEqId(query.get('id'));
      if (moverId && isAdminDetailQuery(query)) { const data = await djangoRequest<T>(`/api/accounts/admin/movers/${encodeURIComponent(moverId)}/`); return { handled: true, data: [data] as T }; }
      ...
      const movers = await djangoRequest<Array<{ id: string; is_available: boolean; approval_status: string; driver_full_name: string; operating_city: string; vehicle_type: string; [key: string]: unknown }>>('/api/core/movers/');
      ...
      const city = getEqId(query.get('operating_city')); if (city) rows = rows.filter((mover) => mover.operating_city === city);
      const vehicle = getEqId(query.get('vehicle_type')); if (vehicle) rows = rows.filter((mover) => mover.vehicle_type === vehicle);
      const searchTerms = getIlikeTerms(query.get('or')); if (searchTerms.length) rows = rows.filter((mover) => { const name = String(mover.driver_full_name || '').toLowerCase(); const operatingCity = String(mover.operating_city || '').toLowerCase(); return searchTerms.some((term) => name.includes(term) || operatingCity.includes(term)); });
      rows.sort((a, b) => String(b.created_at || '').localeCompare(String(a.created_at || ''))); return { handled: true, data: rows as T };
    }
...
export const protectedFunctionPost = async <T = unknown>(functionPath: string, body: unknown): Promise<T> => {
  ...
  const response = await fetch(`${getFunctionUrl()}${functionPath}`, { method: 'POST', credentials: 'include', headers: { apikey: getPublishableKey(), 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
