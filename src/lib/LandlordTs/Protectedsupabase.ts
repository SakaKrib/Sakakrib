...
export const supabase = {
  ...
  rpc: <T = any>(name: string, params?: Record<string, unknown>) => rpc<T>(name, params),
  functions: {
    invoke: async <T = any>(
      slug: string,
      ...
      try {
        const data = await protectedFunctionPost<T>(`/${slug}`, options?.body ?? {});
        return { data, error: null };
      } catch (err) {
        return {
          data: null,
          error: {
            message: err instanceof Error ? err.message : 'Function request failed',
          },
        };
      }
    },
