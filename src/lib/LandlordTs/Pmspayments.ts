...
export interface PMSMpesaCheckoutResponse { success: boolean; error?: string; invoice_id?: string; subscription_id?: string; plan?: string; billing_cycle?: PMSBillingCycle; amount_kes?: number; checkout_request_id?: string; merchant_request_id?: string | null; customer_message?: string; }
export interface PMSPayPalCheckoutResponse { success: boolean; error?: string; subscription_id?: string; paypal_subscription_id?: string; approval_url?: string | null; plan_name?: string; audience?: PMSCheckoutAudience; billing_cycle?: PMSBillingCycle; invoice_id?: string; }
...
export async function initiateMpesaSubscriptionCheckout(audience: PMSCheckoutAudience, planId: string, billingCycle: PMSBillingCycle): Promise<PMSMpesaCheckoutResponse> {
  ...
  if (!data?.success) throw new Error(data?.error || 'Unable to initiate M-Pesa payment.');
  return data;
}
...
export async function initiatePayPalSubscriptionCheckout(planId: string, billingCycle: PMSBillingCycle): Promise<PMSPayPalCheckoutResponse> {
  ...
  if (!data?.success) throw new Error(data?.error || 'Unable to initiate PayPal subscription checkout.');
  return data;
}
...
export async function pollMpesaInvoiceStatus(invoiceId: string, { maxAttempts = 40, intervalMs = 3000 }: { maxAttempts?: number; intervalMs?: number } = {}): Promise<PMSInvoiceStatus> {
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      // Keep polling; the backend remains authoritative and a transient read failure
      // must not turn a valid payment into a false failure state.
    }
