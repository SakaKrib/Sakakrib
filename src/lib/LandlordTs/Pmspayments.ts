...
export async function initiateMpesaSubscriptionCheckout(
  ...
  if (!data?.success) {
    throw new Error(data?.detail || data?.error || 'Unable to initiate M-Pesa payment.');
  }
  return data;
}
...
export async function initiatePayPalSubscriptionCheckout(
  ...
  if (!data?.success) {
    throw new Error(data?.detail || data?.error || 'Unable to start PayPal subscription checkout.');
  }
  return data;
}

export async function approvePayPalSubscription(
  invoiceId: string,
  paypalSubscriptionId: string,
): Promise<PMSPayPalCheckoutResponse> {
  const data = await protectedPost<PMSPayPalCheckoutResponse>('/api/subscriptions/paypal/approve/', {
    invoice_id: invoiceId,
    paypal_subscription_id: paypalSubscriptionId,
  });
  if (!data?.success) {
    throw new Error(data?.detail || data?.error || 'Unable to confirm PayPal subscription.');
  }
  return data;
}
