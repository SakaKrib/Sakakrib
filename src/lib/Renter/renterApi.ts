...
export const renterApi = {
  ...
  submitRentPayment: async (invoiceId: string, transactionReference: string) => {
    ...
    return protectedPost<{ success: boolean; submission_id?: string; invoice_id?: string; status?: string }>('/rest/v1/rpc/submit_rent_payment', {
      p_invoice_id: invoiceId, p_transaction_reference: reference,
    });
  },

  getPaymentDestination: (paymentMethodId: string, unitId: string) =>
    protectedPost<RentPaymentDestination | null>('/rest/v1/rpc/get_rent_payment_destination', { p_payment_method_id: paymentMethodId, p_unit_id: unitId }),

  getRentSummary: (associationId: string) =>
    protectedPost<Record<string, unknown>>('/rest/v1/rpc/get_renter_rent_summary', { p_renter_assoc_id: associationId }),

  getPaymentHistory: (associationId: string) =>
    protectedPost<Array<{ id: string; amount_kes: number; period_year: number; period_month: number; status: string; payment_provider: string | null; payment_method: string | null; mpesa_receipt: string | null; paid_at: string | null; created_at: string }>>('/rest/v1/rpc/get_renter_payment_history', { p_assoc_id: associationId }),
  ...
  getMoverScheduleAvailability: (bookingId: string, from: string, to: string) =>
    protectedPost<MoverScheduleAvailability>('/rest/v1/rpc/get_mover_schedule_availability', { p_booking_id: bookingId, p_from: from, p_to: to }),
