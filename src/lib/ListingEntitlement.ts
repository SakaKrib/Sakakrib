...
interface RawEntitlement { authorized?: boolean; authorized_landlord?: boolean; authorized_real_estate?: boolean; role?: string; reason?: string; free_limit?: number | string | null; free_listings_used?: number | string | null; free_listings_remaining?: number | string | null; subscription_id?: string | null; subscription_plan?: string | null; subscription_status?: string | null; subscription_limit?: number | string | null; subscription_listings_used?: number | string | null; subscription_listings_remaining?: number | string | null; individual_paid_listings?: number | string | null; individual_listing_price_kes?: number | string | null; can_start_listing?: boolean; can_create?: boolean; requires_subscription?: boolean; requires_individual_payment?: boolean; pms_access?: boolean; upgrade_available?: boolean; upgrade_target?: string | null; }
function normalizeStatus(raw: unknown): SubscriptionStatus { const value = typeof raw === 'string' ? raw.trim().toLowerCase() : ''; if (value === 'trial') return 'trial'; if (value === 'active') return 'active'; if (['pending_payment', 'pending payment', 'pending-payment'].includes(value)) return 'pending_payment'; if (['grace_period', 'grace period', 'grace-period'].includes(value)) return 'grace_period'; if (value === 'expired') return 'expired'; if (['cancelled', 'canceled'].includes(value)) return 'cancelled'; return 'none'; }
...
function normalizeEntitlement(role: ListingRole, raw: RawEntitlement): ListingEntitlement { return { role, canStartListing: Boolean(raw.can_start_listing), canCreate: Boolean(raw.can_create), requiresSubscription: Boolean(raw.requires_subscription), requiresIndividualPayment: Boolean(raw.requires_individual_payment), free_limit: numberValue(raw.free_limit), free_listings_used: numberValue(raw.free_listings_used), free_listings_remaining: numberValue(raw.free_listings_remaining), subscriptionId: nullableString(raw.subscription_id), subscriptionPlan: nullableString(raw.subscription_plan), subscriptionStatus: normalizeStatus(raw.subscription_status), subscriptionLimit: nullableNumber(raw.subscription_limit), subscriptionListingsUsed: numberValue(raw.subscription_listings_used), subscriptionListingsRemaining: nullableNumber(raw.subscription_listings_remaining), individualPaidListings: numberValue(raw.individual_paid_listings), individualListingPriceKes: numberValue(raw.individual_listing_price_kes), pmsAccess: Boolean(raw.pms_access), upgradeAvailable: Boolean(raw.upgrade_available), upgradeTarget: nullableString(raw.upgrade_target) }; }
export async function fetchLandlordListingEntitlement(_landlordId?: string): Promise<ListingEntitlement> { const raw = await protectedGet<RawEntitlement>('/api/listings/entitlement/'); if (raw.authorized === false || raw.authorized_landlord === false) throw new Error(raw.reason || 'This account is not authorized for landlord listings.'); return normalizeEntitlement('landlord', raw); }
export async function fetchRealEstateListingEntitlement(_realEstateId?: string): Promise<ListingEntitlement> { const raw = await protectedGet<RawEntitlement>('/api/listings/entitlement/'); if (raw.authorized === false || raw.authorized_real_estate === false) throw new Error(raw.reason || 'This account is not authorized for real estate listings.'); return normalizeEntitlement('real_estate', raw); }
...
export async function fetchSubscriptionPlans(role: ListingRole): Promise<SubscriptionPlan[]> { const rows = await protectedGet<SubscriptionPlan[]>(`/api/subscriptions/plans/?audience=${role === 'landlord' ? 'LANDLORD' : 'REAL_ESTATE'}`); return (rows ?? []).map((row) => ({ ...row, id: String(row.id), name: String(row.name), audience: row.audience === 'REAL_ESTATE' ? 'REAL_ESTATE' : 'LANDLORD', monthly_price_kes: numberValue(row.monthly_price_kes), annual_price_kes: numberValue(row.annual_price_kes), max_listings: nullableNumber(row.max_listings), max_units_per_listing: nullableNumber(row.max_units_per_listing) })); }
