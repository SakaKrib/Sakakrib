...
import { AlertCircle, Check, CheckCircle2, Crown, Loader2, Smartphone, X, XCircle } from 'lucide-react';
...
export default function PMSCheckoutModal({ open, onOpenChange, audience, plan, onSuccess }: PMSCheckoutModalProps) {
  ...
  const startPayPal = async () => {
    ...
    try {
      ...
      if (!result.paypal_subscription_id || !result.invoice_id) {
        throw new Error('PayPal checkout did not return the required server references.');
      }
      setInvoiceId(result.invoice_id);
      setStep('paypal-waiting');
    } catch (err) {
      if (cancelledRef.current) return;
      setError(err instanceof Error ? err.message : 'Unable to prepare PayPal checkout.');
      setStep('failed');
    } finally {
      setStarting(false);
    }
  ...
  const handlePayPalSuccess = async () => {
    // The reusable PayPal button performs its own server-side approval.
    // We only re-read Django state here; the browser never marks the subscription active.
      ...
      <div className="card relative w-full max-w-md overflow-hidden rounded-2xl shadow-xl">
        <div className="flex items-start justify-between border-b border-gray-200 p-5 dark:border-gray-800">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 id="pms-checkout-title" className="text-lg font-bold text-gray-900 dark:text-white">Complete payment</h2>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{plan.planName} · {plan.billingCycle === 'MONTHLY' ? 'Monthly' : 'Annual'} · {formatKES(plan.amountKes)}</p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <button type="button" aria-pressed={method === 'MPESA'} onClick={() => setMethod('MPESA')} className={`rounded-2xl border p-4 text-left transition ${method === 'MPESA' ? 'border-brand-600 bg-brand-50 ring-2 ring-brand-600/10 dark:border-brand-500 dark:bg-brand-900/20' : 'border-gray-200 hover:border-brand-400 dark:border-gray-700'}`}>
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">STK Push</p>
                </button>
                <button type="button" aria-pressed={method === 'PAYPAL'} onClick={() => setMethod('PAYPAL')} className={`rounded-2xl border p-4 text-left transition ${method === 'PAYPAL' ? 'border-brand-600 bg-brand-50 ring-2 ring-brand-600/10 dark:border-brand-500 dark:bg-brand-900/20' : 'border-gray-200 hover:border-brand-400 dark:border-gray-700'}`}>
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Secure recurring payment</p>
                </button>
              </div>

              {method === 'MPESA' && (
                <div className="space-y-2">
                  <label htmlFor="pms-mpesa-phone" className="text-sm font-semibold text-gray-900 dark:text-white">M-Pesa phone number</label>
                  <input id="pms-mpesa-phone" value={phoneNumber} onChange={event => setPhoneNumber(event.target.value)} inputMode="tel" autoComplete="tel" placeholder="2547XXXXXXXX" className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white" />
                  <p className="text-xs text-gray-500 dark:text-gray-400">The STK Push will be sent to this number.</p>
                </div>
              )}
              ...
              {method === 'MPESA' ? (
                <button type="button" disabled={starting} onClick={startMpesa} className="btn-primary inline-flex w-full items-center justify-center gap-2 disabled:opacity-50">
                  {starting ? <><Loader2 className="h-4 w-4 animate-spin" />Starting STK Push…</> : <>Send STK Push</>}
                </button>
              ) : (
                <div className="space-y-2">
                  <PayPalPaymentButton mode="SUBSCRIPTION" planId={plan.planId} billingCycle={plan.billingCycle} onSuccess={handlePayPalSuccess} onError={message => { setError(message); setStep('failed'); }} disabled={starting} />
                  <p className="text-center text-xs text-gray-500 dark:text-gray-400">PayPal will open its secure approval experience here.</p>
                </div>
              )}
            </div>
          )}
