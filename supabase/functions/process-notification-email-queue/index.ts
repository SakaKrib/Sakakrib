...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (fetchError) {
      ...
      return jsonResponse({ success: false, error: "Unable to load queue" }, 500);
    }
    ...
    for (const email of pending ?? []) {
      try {
        ...
        if (!resendResponse.ok) {
          ...
          await supabase.from("notification_emails").update({ status: "failed" }).eq("id", email.id);
          failed++;
          continue;
        }
    ...
    return jsonResponse({ success: true, sent, failed, total: (pending ?? []).length });
  } catch (error) {
