...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!type || !(type in EMAIL_SUBJECTS)) return json({
      success: false,
      error: `Unknown or missing email type: ${type || ""}`
    }, 400);
    let recipient = "";
    ...
    if (type === "otp_verification") {
      const { error: issueError } = await db.rpc("issue_signup_otp", {
        p_email: recipient
      });
      if (issueError) throw new Error(issueError.message || "Unable to issue the verification code.");
      const { data: otp, error: otpError } = await db.rpc("get_signup_otp_for_email", {
        p_email: recipient
      });
    ...
    const { data: notification, error: notificationError } = await db.from("notification_emails").insert({
      recipient,
      subject,
      html_body: htmlBody,
      template_type: type,
    ...
    if (!resendResponse.ok) {
      ...
      throw new Error(resendData?.message || resendData?.error || `Resend failed with status ${resendResponse.status}.`);
    }
    await db.from("notification_emails").update({
      status: "sent",
