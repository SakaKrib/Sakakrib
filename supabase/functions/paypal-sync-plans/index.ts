...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: plans, error: plansError } = await supabase
      ...
      .select("id, name, audience, paypal_product_id, paypal_monthly_plan_id, paypal_annual_plan_id, monthly_price_kes, annual_price_kes, paypal_monthly_price_usd, paypal_annual_price_usd")
      .order("audience")
      ...
      try {
        if (plan.paypal_monthly_plan_id) {
          ...
          if (detailResponse.ok) {
            ...
            results.push({ plan_id: plan.id, cycle: "MONTHLY", paypal_status: detail.status, paypal_plan_id: plan.paypal_monthly_plan_id });
          }
        }
        if (plan.paypal_annual_plan_id) {
          ...
          if (detailResponse.ok) {
            const detail = await detailResponse.json();
            results.push({ plan_id: plan.id, cycle: "ANNUAL", paypal_status: detail.status, paypal_plan_id: plan.paypal_annual_plan_id });
          }
        }
      } catch (err) {
        results.push({ plan_id: plan.id, error: err instanceof Error ? err.message : "lookup failed" });
      }
    }

    return jsonResponse({ success: true, plans_synced: results.length, results });
  } catch (error) {
