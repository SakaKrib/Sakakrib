...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!payment_intent_id) {
      return jsonResponse({ success: false, error: "payment_intent_id is required" }, 400);
    }

    const { data: intent, error: intentError } = await supabase
      .from("listing_payment_intents")
      .select("id, user_id, role, amount_kes, status, expires_at, provider_reference")
      .eq("id", payment_intent_id)
      .maybeSingle();

    if (intentError) throw new Error("Unable to load payment intent");
    if (!intent) return jsonResponse({ success: false, error: "Payment intent not found" }, 404);
    if (intent.user_id !== user.id) return jsonResponse({ success: false, error: "Not your payment intent" }, 403);
    if (intent.status !== "PENDING") return jsonResponse({ success: false, error: `Payment intent is ${intent.status.toLowerCase()}` }, 409);
    if (intent.expires_at && new Date(intent.expires_at).getTime() <= Date.now()) {
      await supabase
        ...
        .eq("id", intent.id)
        .eq("status", "PENDING");
      return jsonResponse({ success: false, error: "Payment intent has expired" }, 409);
    }
