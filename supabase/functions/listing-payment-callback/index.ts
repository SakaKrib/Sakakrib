...
function getMetadataValue(items, name) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!callback) return jsonResponse({
      success: false,
      error: "Invalid callback payload"
    }, 400);
    const { MerchantRequestID, CheckoutRequestID, ResultCode, ResultDesc, CallbackMetadata } = callback;
    if (!CheckoutRequestID) return jsonResponse({
      success: false,
      error: "CheckoutRequestID missing"
    }, 400);
    const { data: intent, error: intentError } = await supabase.from("listing_payment_intents").select("id, user_id, amount_kes, status, provider_reference").eq("provider_reference", CheckoutRequestID).maybeSingle();
    ...
    if (!mpesaReceipt) return jsonResponse({
      success: false,
      error: "M-Pesa receipt missing"
    }, 400);
    if (paidAmount === null) return jsonResponse({
      success: false,
    ...
    const { data: result, error: processError } = await supabase.rpc("process_listing_payment", {
