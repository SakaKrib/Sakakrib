...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { plan_id, billing_cycle, audience } = body;
    ...
    const { data: plan, error: planError } = await supabase.from("subscription_plans").select("id, name, audience, paypal_monthly_price_usd, paypal_annual_price_usd, paypal_product_id").eq("id", plan_id).maybeSingle();
    ...
    if (!Number.isFinite(amountUsd) || amountUsd <= 0) {
      return jsonResponse({
        success: false,
        error: "Plan pricing not configured for PayPal"
      }, 400);
    }
