...
const publicClient = createClient(BASE_URL, SUPABASE_ANON_KEY, {
  auth: {
...
Deno.serve(async (request)=>{
  ...
  const origin = request.headers.get('origin');
  const configured = Deno.env.get('APP_ORIGIN');
  if (configured && origin && origin !== configured) return json(request, {
    error: 'Origin not allowed.'
  }, 403);
  try {
    const user = await authenticate(request);
    if (!user) return json(request, {
      authenticated: false,
    ...
    if (action === 'sign') {
      ...
      if (!messageId) return json(request, {
      ...
      if (messageError || !message) return json(request, {
        error: 'Attachment message not found.'
      }, 404);
      if (message.sender_id !== user.id && message.receiver_id !== user.id) return json(request, {
        error: 'Not authorized to access this attachment.'
      }, 403);
      const rawAttachments = message.event_data?.attachments;
      if (!Array.isArray(rawAttachments) || rawAttachments.length === 0) return json(request, {
        error: 'No attachment found.'
      }, 404);
      const attachment = rawAttachments[0];
      const path = typeof attachment.path === 'string' ? attachment.path : '';
      if (!path) return json(request, {
        error: 'Invalid attachment path.'
      }, 400);
      const { data: signed, error: signError } = await admin.storage.from(BUCKET).createSignedUrl(path, 3600);
      if (signError || !signed?.signedUrl) return json(request, {
        error: 'Unable to sign attachment.'
      }, 500);
      return json(request, {
        signed_url: signed.signedUrl
      });
    }
    if (!(file instanceof File)) return json(request, {
      error: 'Image file is required.'
    }, 400);
    if (!ALLOWED_TYPES.has(file.type)) return json(request, {
      error: 'Only JPG, PNG, WEBP, and GIF images are allowed.'
    }, 415);
    if (file.size <= 0 || file.size > MAX_BYTES) return json(request, {
    ...
    if (uploadError) return json(request, {
      error: 'Unable to upload image.'
    }, 500);
    const { data: signed, error: signError } = await admin.storage.from(BUCKET).createSignedUrl(path, 3600);
    if (signError || !signed?.signedUrl) return json(request, {
      error: 'Image uploaded but could not be signed.'
    }, 500);
    return json(request, {
      path,
      name: file.name,
