...
const adminSupabase = SUPABASE_SERVICE_ROLE_KEY ? createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  auth: {
    persistSession: false,
    autoRefreshToken: false,
    detectSessionInUrl: false
...
const allowedOrigin = (request)=>{
  ...
  if (origin === 'http://localhost:5174' || origin === 'http://localhost:5173' || origin === 'http://127.0.0.1:5174' || origin === 'http://100.109.224.0:5174' || origin === 'http://100.109.224.0:5173') return origin;
...
const establishVerifiedSession = async (email)=>{
  if (!adminSupabase) throw new Error('Server authentication configuration is incomplete.');
  const { data: linkData, error: linkError } = await adminSupabase.auth.admin.generateLink({
    type: 'magiclink',
    email
  });
  if (linkError || !linkData?.properties?.hashed_token) throw new Error(linkError?.message ?? 'Unable to establish the authenticated session.');
  const { data: sessionData, error: sessionError } = await supabase.auth.verifyOtp({
    token_hash: linkData.properties.hashed_token,
    type: 'email'
  });
  if (sessionError || !sessionData.session || !sessionData.user) throw new Error(sessionError?.message ?? 'Unable to establish the authenticated session.');
  return sessionData;
};
const authenticatedResponse = async (request, accessToken)=>{
  const { data, error } = await supabase.auth.getUser(accessToken);
  if (error || !data.user) return json(request, {
    authenticated: false
  }, 401);
  const profile = await getProfile(accessToken, data.user.id);
  if (!profile || profile.email_verified !== true) return json(request, {
    authenticated: false,
...
const refreshFromCookie = async (request, refreshToken)=>{
  const { data, error } = await supabase.auth.refreshSession({
    refresh_token: refreshToken
  });
  if (error || !data.session || !data.user) return json(request, {
    authenticated: false
  }, 401, clearAuthCookies());
...
Deno.serve(async (request)=>{
  ...
  if (request.method !== 'POST') return json(request, {
    error: 'Method not allowed.'
  }, 405);
  const origin = request.headers.get('origin');
  const configuredOrigin = Deno.env.get('APP_ORIGIN');
  if (configuredOrigin && origin && origin !== configuredOrigin) return json(request, {
  ...
  try {
    ...
    if (action === 'signup') {
      ...
      if (!email || !password || !fullName) return json(request, {
      ...
      if (Array.isArray(data.user.identities) && data.user.identities.length === 0) return json(request, {
        error: 'An account with this email already exists. Please log in.'
      }, 409);
      if (data.session) {
        const profile = await getProfile(data.session.access_token, data.user.id);
        if (profile?.email_verified === true) return json(request, {
          authenticated: true,
          user: {
            id: data.user.id,
    ...
    if (action === 'login') {
      const email = typeof payload.email === 'string' ? payload.email.trim().toLowerCase() : '';
      const password = typeof payload.password === 'string' ? payload.password : '';
      if (!email || !password) return json(request, {
    ...
    if (action === 'session') {
      ...
      if (cookies.sk_access) {
        const sessionResponse = await authenticatedResponse(request, cookies.sk_access);
        if (sessionResponse.status !== 401) return sessionResponse;
      }
      if (!cookies.sk_refresh) return json(request, {
        authenticated: false
      }, 401, clearAuthCookies());
      return refreshFromCookie(request, cookies.sk_refresh);
    }
    if (action === 'refresh') {
      const refreshToken = readCookies(request).sk_refresh;
      if (!refreshToken) return json(request, {
        authenticated: false
      }, 401, clearAuthCookies());
    ...
    if (action === 'verify_otp') {
      ...
      if (!email || otp.length !== 6) return json(request, {
      ...
      if (error) return json(request, {
        error: error.message || 'Invalid or expired verification code.'
      }, 400);
      const result = data;
      if (!result?.success) return json(request, {
      ...
      if (!profile || profile.email_verified !== true) {
        ...
        return json(request, {
          error: 'Email verification succeeded, but the authenticated profile could not be established.'
        }, 500, clearAuthCookies());
      }
      return json(request, {
        success: true,
        authenticated: true,
        ...
        profile
      }, 200, setAuthCookies(sessionData.session.access_token, sessionData.session.refresh_token));
    }
    if (action === 'logout') {
