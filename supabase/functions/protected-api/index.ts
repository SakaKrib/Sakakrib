...
const normalize = (path)=>{
  ...
  return i >= 0 ? p.slice(i) : p === '/rest/v1' ? '/rest/v1/' : p;
};
const safe = (p)=>p.startsWith('/rest/v1/') && !p.includes('://') && !p.startsWith('//') && !p.includes('\\') && !p.includes('\0');
const forwardResponse = async (response, req, refreshHeaders)=>{
  ...
  if (response.status === 204 || response.status === 205 || response.status === 304) return new Response(null, {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (pathname === subscriptionStkPrefix || pathname === `/${FUNCTION_NAME}/subscription-stk` || pathname.endsWith('/subscription-stk')) {
      ...
      const authz = authorize(a.profile, '/rest/v1/landlord/subscription-stk');
      ...
      const response = await fetch(`${BASE}/functions/v1/subscription-stk`, {
        method: 'POST',
        headers,
        body
      });
      return await forwardResponse(response, req, refreshHeaders);
    }
    ...
    if (pathname === storagePrefix || pathname === `/${FUNCTION_NAME}/storage/upload` || pathname.endsWith('/storage/upload')) {
      ...
      const { error: ue } = await storageClient.storage.from(bucket).upload(normalized, await file.arrayBuffer(), {
    ...
    if (pathname === signPrefix || pathname === `/${FUNCTION_NAME}/storage/sign` || pathname.endsWith('/storage/sign')) {
      ...
      const authz = authorize(a.profile, '/rest/v1/storage/sign');
      if (!authz.ok) return json(req, {
        authenticated: true,
        authorized: false,
        error: authz.error
      ...
      const isOwner = first === a.user.id;
      const isAdmin = role === 'admin';
      if (!isOwner && !isAdmin) return json(req, {
        error: 'You may only view your own documents.'
      }, 403, refreshHeaders);
      if (normalized.includes('..') || normalized.includes('\\') || normalized.includes('\0')) return json(req, {
        error: 'Invalid storage path.'
      }, 400, refreshHeaders);
      ...
      if (se || !signed?.signedUrl) {
        console.error('Signed URL failed:', se);
        return json(req, {
          error: 'Unable to generate a preview URL.'
        }, 500, refreshHeaders);
      }
      return json(req, {
        ok: true,
    ...
    if (pathname === rolePrefix || pathname === `/${FUNCTION_NAME}/set-role` || pathname.endsWith('/set-role')) {
      ...
      if (a.profile.email_verified !== true) return json(req, {
        authenticated: true,
        authorized: false,
        error: 'Email verification is required.'
      }, 403, refreshHeaders);
      const body = await req.json().catch(()=>null);
      const selectedRole = typeof body?.role === 'string' ? body.role.trim().toLowerCase() : '';
      ...
      if (!allowedRoles.has(selectedRole)) return json(req, {
        error: 'Invalid role selected.'
      }, 400, refreshHeaders);
      const userClient = createClient(BASE, SUPABASE_ANON_KEY, {
        auth: {
          persistSession: false,
          autoRefreshToken: false,
          detectSessionInUrl: false
      ...
      const { data: updated, error: updateError } = await userClient.from('profiles').update({
        role: selectedRole
      }).eq('id', a.user.id).select('id,role').maybeSingle();
      if (updateError) {
        console.error('Role update failed:', updateError);
        return json(req, {
          authenticated: true,
          authorized: true,
          error: updateError.message || 'Unable to save your role.'
        }, 400, refreshHeaders);
      }
      if (!updated) return json(req, {
        authenticated: true,
        authorized: true,
    ...
    // Authenticated email proxy. The email function uses its own service-role client
    // and does not require a Supabase JWT, so protected-api authenticates the caller
    // with the existing HttpOnly cookie first, then forwards the request internally.
    const emailPrefix = `/functions/v1/${FUNCTION_NAME}/send-notification-emails`;
    if (pathname === emailPrefix || pathname === `/${FUNCTION_NAME}/send-notification-emails` || pathname.endsWith('/send-notification-emails')) {
      if (req.method !== 'POST') return json(req, {
        error: 'Method not allowed.'
      }, 405, refreshHeaders);
      const authz = authorize(a.profile, '/rest/v1/notification_emails');
      if (!authz.ok) return json(req, {
        authenticated: true,
        authorized: false,
        role: a.profile.role,
      ...
      headers.set('apikey', SUPABASE_ANON_KEY);
      headers.set('Content-Type', req.headers.get('content-type') ?? 'application/json');
      const response = await fetch(`${BASE}/functions/v1/send-notification-emails`, {
        method: 'POST',
        headers,
        body
      });
    ...
    const publicReadWithoutRole = req.method === 'GET' && (target === '/rest/v1/listings' || target === '/rest/v1/movers' || target === '/rest/v1/reviews' || target === '/rest/v1/profiles' && url.searchParams.get('select') === 'id' && url.searchParams.get('role') === 'eq.landlord' && url.searchParams.get('landlord_application_status') === 'eq.approved');
    const authz = publicReadWithoutRole ? {
      ok: true
    } : authorize(a.profile, target);
    if (!authz.ok) return json(req, {
      authenticated: true,
      authorized: false,
      role: a.profile.role,
