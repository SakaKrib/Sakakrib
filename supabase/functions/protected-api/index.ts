...
const authenticate=async(token:string):Promise<Auth|null>=>{
  const ac=makeAuthClient(); const {data,error}=await ac.auth.getUser(token); if(error||!data.user) return null;
  const uc=createClient(BASE,SUPABASE_ANON_KEY,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false},global:{headers:{Authorization:`Bearer ${token}`}}});
  ...
  if(pe||!p){console.error('Profile authentication lookup failed:',pe);return null;}
  return {user:{id:data.user.id,email:data.user.email??null},accessToken:token,profile:p as Profile};
};
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    // The browser sends only HttpOnly cookies to protected-api.
    // protected-api validates the cookie, then forwards the resulting
    // short-lived user JWT to the target function. The target function
    // keeps verify_jwt enabled and never needs to read the browser cookie.
    ...
    if(pathname===subscriptionStkPrefix || pathname===`/${FUNCTION_NAME}/subscription-stk` || pathname.endsWith('/subscription-stk')){
      ...
      const targetUrl=`${BASE}/functions/v1/subscription-stk`;
      const response=await fetch(targetUrl,{method:'POST',headers,body});
      const rh=new Headers(cors(req)); rh.set('Cache-Control','no-store');
      const contentType=response.headers.get('content-type'); if(contentType) rh.set('content-type',contentType);
      for(const [n,v] of refreshHeaders) rh.append(n,v);
      return new Response(await response.arrayBuffer(),{status:response.status,headers:rh});
    }
    ...
    if(pathname===storagePrefix || pathname===`/${FUNCTION_NAME}/storage/upload` || pathname.endsWith('/storage/upload')){
      ...
      if(!(file instanceof File))return json(req,{error:'file is required.'},400,refreshHeaders);
      if(typeof bucket!=='string'||!['id-documents','licenses','kyc-documents'].includes(bucket))return json(req,{error:'Invalid storage bucket.'},400,refreshHeaders);
      if(typeof path!=='string'||!path)return json(req,{error:'path is required.'},400,refreshHeaders);
      const normalized=path.replace(/^\/+/, ''); const first=normalized.split('/')[0];
      if(first!==a.user.id)return json(req,{error:'You may only upload documents to your own folder.'},403,refreshHeaders);
      if(normalized.includes('..')||normalized.includes('\\')||normalized.includes('\0'))return json(req,{error:'Invalid storage path.'},400,refreshHeaders);
      if(file.size>10*1024*1024)return json(req,{error:'Image is too large. Maximum size is 10 MB.'},413,refreshHeaders);
      if(!file.type.startsWith('image/'))return json(req,{error:'Only image files are allowed.'},415,refreshHeaders);
      const storageClient=createClient(BASE,SUPABASE_ANON_KEY,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false},global:{headers:{Authorization:`Bearer ${access}`}}});
      const bytes=await file.arrayBuffer();
      const {error:ue}=await storageClient.storage.from(bucket).upload(normalized,bytes,{contentType:file.type||'image/jpeg',cacheControl:'3600',upsert:false});
      if(ue){console.error('Storage upload failed:',ue);return json(req,{error:ue.message||'Document upload failed.'},400,refreshHeaders);}
      const {data:signed,error:se}=await storageClient.storage.from(bucket).createSignedUrl(normalized,3600);
      if(se||!signed?.signedUrl){console.error('Signed URL failed:',se);return json(req,{error:'Document uploaded but a preview URL could not be generated.'},500,refreshHeaders);}
      return json(req,{ok:true,bucket,path:normalized,url:signed.signedUrl,publicUrl:signed.signedUrl},200,refreshHeaders);
    }

    // Resolve an already-uploaded document's durable storage path back into a fresh, short-lived signed URL for display.
    ...
    if(pathname===signPrefix || pathname===`/${FUNCTION_NAME}/storage/sign` || pathname.endsWith('/storage/sign')){
      if(req.method!=='POST') return json(req,{error:'Method not allowed.'},405,refreshHeaders);
      const authz=authorize(a.profile,'/rest/v1/storage/sign'); if(!authz.ok)return json(req,{authenticated:true,authorized:false,error:authz.error},authz.status,refreshHeaders);
      const body=await req.json().catch(()=>null);
      const bucket=body?.bucket; const path=body?.path;
      if(typeof bucket!=='string'||!['id-documents','licenses','kyc-documents'].includes(bucket))return json(req,{error:'Invalid storage bucket.'},400,refreshHeaders);
      if(typeof path!=='string'||!path)return json(req,{error:'path is required.'},400,refreshHeaders);
      const normalized=path.replace(/^\/+/, ''); const first=normalized.split('/')[0];
      const role=(a.profile.role??'').trim().toLowerCase();
      const isOwner=first===a.user.id; const isAdmin=role==='admin';
      if(!isOwner&&!isAdmin)return json(req,{error:'You may only view your own documents.'},403,refreshHeaders);
      if(normalized.includes('..')||normalized.includes('\\')||normalized.includes('\0'))return json(req,{error:'Invalid storage path.'},400,refreshHeaders);
      const storageClient=createClient(BASE,SUPABASE_ANON_KEY,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false},global:{headers:{Authorization:`Bearer ${access}`}}});
      const {data:signed,error:se}=await storageClient.storage.from(bucket).createSignedUrl(normalized,3600);
      if(se||!signed?.signedUrl){console.error('Signed URL failed:',se);return json(req,{error:'Unable to generate a preview URL.'},500,refreshHeaders);}
      return json(req,{ok:true,bucket,path:normalized,url:signed.signedUrl},200,refreshHeaders);
    }

    const target=normalize(pathname); if(!safe(target))return json(req,{error:'Unsupported protected API path.'},400,refreshHeaders);
    const authz=authorize(a.profile,target); if(!authz.ok)return json(req,{authenticated:true,authorized:false,role:a.profile.role,error:authz.error},authz.status,refreshHeaders);
    const targetUrl=`${BASE}${target}${url.search}`; const body=req.method==='GET'||req.method==='HEAD'?undefined:await req.text();
    const headers=new Headers(); headers.set('apikey',SUPABASE_ANON_KEY); headers.set('Authorization',`Bearer ${access}`); headers.set('Accept',req.headers.get('accept')??'application/json');
    for(const name of ['content-type','prefer','range']){const v=req.headers.get(name);if(v)headers.set(name,v);}
    let response=await fetch(targetUrl,{method:req.method,headers,body});
    ...
    const rh=new Headers(cors(req));rh.set('Cache-Control','no-store');for(const n of ['content-type','content-range','location']){const v=response.headers.get(n);if(v)rh.set(n,v);}for(const [n,v] of refreshHeaders)rh.append(n,v);if(response.status===401)for(const [n,v] of clearCookies())rh.append(n,v);
    return new Response(await response.arrayBuffer(),{status:response.status,headers:rh});
