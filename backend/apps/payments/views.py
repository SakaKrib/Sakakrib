...
def _paypal_capture_details(raw):
    ...
    return capture_id, amount_data.get('value'), amount_data.get('currency_code')
...
class ListingPaymentStartView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            intent = ListingPaymentIntent.objects.select_for_update().filter(pk=intent_id, user_id=request.user.pk, status='PENDING').first()
            if not intent:
            ...
            if existing_reference:
                return Response({'success': True, 'already_started': True, 'payment_intent_id': str(intent.id),
                                 'provider': existing_provider or provider_name, 'provider_reference': existing_reference,
            ...
            if provider_name == 'mpesa':
                supplied_phone = request.data.get('phone_number')
                if not supplied_phone:
                    return Response({'detail': 'A phone number is required for M-Pesa STK Push.'}, status=400)
                try:
                    phone_number = _normalize_kenyan_phone(supplied_phone)
                except ValueError as exc:
                    return Response({'detail': str(exc)}, status=400)
                amount, currency, fx_rate = intent.amount_kes, 'KES', None
            else:
                phone_number = None
                try:
            ...
            intent.updated_at = timezone.now()
            fields = ['provider', 'provider_reference', 'provider_amount', 'provider_currency', 'paypal_fx_rate', 'updated_at']
            if provider_name == 'paypal':
                fields.append('paypal_order_id')
...
class MpesaListingCallbackView(APIView):
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                intent = ListingPaymentIntent.objects.select_for_update().filter(provider='MPESA', provider_reference=checkout_id, status='PENDING').first()
                if not intent:
                    return Response({'ResultCode': 0, 'ResultDesc': 'Accepted'})
...
class PayPalListingCaptureView(APIView):
    ...
    def post(self, request):
        ...
        if intent.status == 'PAID':
            return Response({'success': True, 'already_processed': True, 'payment_intent_id': str(intent.id), 'listing_id': str(intent.listing_id) if intent.listing_id else None, 'status': 'PAID'})
