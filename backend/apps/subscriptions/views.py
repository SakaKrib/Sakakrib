...
class MySubscriptionInvoiceView(APIView):
    def get(self, request, invoice_id):
        ...
        return Response({
            ...
            'merchant_request_id': invoice.merchant_request_id,
            'mpesa_receipt': invoice.mpesa_receipt,
            'paid_at': invoice.paid_at,
            'listing_id': str(invoice.listing_id) if invoice.listing_id else None,
            'result_code': invoice.result_code,
            'result_description': invoice.result_description,
        })
...
class PayPalSubscriptionApproveView(APIView):
    """Record the PayPal subscription ID; the webhook remains authoritative for payment."""
    ...
    def post(self, request):
        ...
        try:
            result = record_paypal_subscription_approval(invoice.id, paypal_subscription_id)
        except ValueError as exc:
            return Response({'success': False, 'detail': str(exc)}, status=400)
        except RuntimeError as exc:
            return Response({'success': False, 'detail': str(exc)}, status=502)
