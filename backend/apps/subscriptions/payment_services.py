...
def create_subscription_checkout(profile, plan_id, billing_cycle, provider, phone_number=None, listing_id=None):
    ...
    if billing_cycle not in ('MONTHLY', 'ANNUAL'): raise ValueError('billing_cycle must be MONTHLY or ANNUAL')
    if provider not in ('mpesa', 'paypal'): raise ValueError('provider must be mpesa or paypal')
    if profile.role not in ('landlord', 'real_estate'): raise ValueError('Only landlord and real estate accounts can subscribe')
    if profile.verification_status != 'verified': raise ValueError('Identity verification is required before subscription checkout')
    if profile.role == 'landlord' and profile.landlord_application_status != 'approved': raise ValueError('Landlord application approval is required before subscription checkout')
    if profile.role == 'real_estate' and profile.real_estate_application_status != 'approved': raise ValueError('Real estate application approval is required before subscription checkout')
    plan = SubscriptionPlan.objects.filter(pk=plan_id, audience=profile.role.upper()).first()
    if not plan: raise ValueError('Subscription plan not found for this account type')
    draft = None
    if listing_id:
        from apps.listings.models import Listing
        draft = Listing.objects.filter(pk=listing_id, user_id=profile.id, is_draft=True).first()
        if not draft: raise ValueError('Listing draft not found or not owned by this account')
        if not draft.is_property_management: raise ValueError('A listing subscription checkout must be attached to a property-management draft.')
    paypal_plan_id = plan.paypal_monthly_plan_id if billing_cycle == 'MONTHLY' else plan.paypal_annual_plan_id
    if provider == 'paypal' and not paypal_plan_id: raise ValueError('PayPal is not configured for this plan yet')
    if provider == 'mpesa' and not phone_number: raise ValueError('phone_number is required for M-Pesa')
    subscription_model = LandlordSubscription if profile.role == 'landlord' else RealEstateSubscription; owner_field = 'landlord_id' if profile.role == 'landlord' else 'real_estate_id'
    if subscription_model.objects.filter(**{owner_field: profile.id}, status__in=('ACTIVE', 'GRACE_PERIOD')).exists(): raise ValueError('You already have an active subscription. Use the upgrade or renewal flow.')
    amount_kes = plan.annual_price_kes if billing_cycle == 'ANNUAL' else plan.monthly_price_kes; amount_usd = plan.paypal_annual_price_usd if billing_cycle == 'ANNUAL' else plan.paypal_monthly_price_usd; now = timezone.now()
    with transaction.atomic():
        ...
        create_kwargs = {owner_field: profile.id, 'plan_id': plan.id, 'billing_cycle': billing_cycle, 'status': 'PENDING_PAYMENT', 'grace_period_end': None, 'auto_renew': False, 'created_at': now, 'updated_at': now, 'billing_amount_kes': amount_kes, 'billing_amount_usd': amount_usd, 'paypal_plan_id': paypal_plan_id if provider == 'paypal' else None}
        create_kwargs['current_period_start'] = now if profile.role == 'landlord' else None; create_kwargs['current_period_end'] = now if profile.role == 'landlord' else None
        subscription = subscription_model.objects.create(**create_kwargs)
        invoice = SubscriptionInvoice.objects.create(amount_kes=amount_kes, status='PENDING', payment_provider=provider.upper(), payment_method=provider.upper(), phone_number=phone_number, landlord_subscription_id=subscription.id if profile.role == 'landlord' else None, real_estate_subscription_id=subscription.id if profile.role == 'real_estate' else None, currency='KES' if provider == 'mpesa' else 'USD', amount_usd=amount_usd, billing_period_start=None, billing_period_end=None, pricing_snapshot_source='subscription_plans', listing=draft)
    if provider == 'paypal':
        from .paypal_subscription_services import create_paypal_subscription
        try:
            remote = create_paypal_subscription(plan_id=paypal_plan_id, custom_id=str(invoice.id), return_url=_paypal_return_url(settings.PAYPAL_SUBSCRIPTION_RETURN_URL, invoice.id), cancel_url=_paypal_return_url(settings.PAYPAL_SUBSCRIPTION_CANCEL_URL, invoice.id)); paypal_subscription_id = str(remote['id']); approval_url = next(str(link['href']) for link in remote.get('links', []) if link.get('rel') == 'approve' and link.get('href'))
            subscription.paypal_subscription_id = paypal_subscription_id; subscription.paypal_status = str(remote.get('status') or 'APPROVAL_PENDING'); subscription.auto_renew = False; subscription.updated_at = timezone.now(); subscription.save(update_fields=['paypal_subscription_id', 'paypal_status', 'auto_renew', 'updated_at'])
            invoice.paypal_subscription_id = paypal_subscription_id; invoice.provider_reference = paypal_subscription_id; invoice.save(update_fields=['paypal_subscription_id', 'provider_reference'])
            return {'success': True, 'subscription_id': str(subscription.id), 'invoice_id': str(invoice.id), 'listing_id': str(draft.id) if draft else None, 'provider': 'paypal', 'billing_cycle': billing_cycle, 'plan_id': str(plan.id), 'plan_name': plan.name, 'paypal_plan_id': paypal_plan_id, 'paypal_subscription_id': paypal_subscription_id, 'approval_url': approval_url, 'status': 'PENDING_PAYMENT', 'payment_action': 'PAYPAL_SUBSCRIPTION_APPROVAL'}
        except Exception:
            invoice.status = 'FAILED'; invoice.result_description = 'Unable to create PayPal subscription checkout.'; invoice.save(update_fields=['status', 'result_description']); subscription.status = 'CANCELLED'; subscription.updated_at = timezone.now(); subscription.save(update_fields=['status', 'updated_at']); raise
    result = get_provider('mpesa').create_payment(amount=Decimal(amount_kes), currency='KES', reference=str(invoice.id), metadata={'phone_number': phone_number, 'description': 'SakaKrib subscription'})
    if not result.success:
        invoice.status = 'FAILED'; invoice.result_description = result.message; invoice.save(update_fields=['status', 'result_description']); subscription.status = 'CANCELLED'; subscription.updated_at = timezone.now(); subscription.save(update_fields=['status', 'updated_at']); raise RuntimeError(result.message)
...
def finalize_mpesa_subscription(invoice_id, result_code, result_description='', mpesa_receipt=None, checkout_request_id=None, merchant_request_id=None, phone_number=None, paid_amount=None):
    with transaction.atomic():
        ...
        if not invoice: return {'success': True, 'already_settled': True}
        ...
        if result_code_int is None: raise ValueError('M-Pesa result code is required')
        if not mpesa_receipt: raise ValueError('M-Pesa receipt is required for successful settlement')
        reference = (checkout_request_id or invoice.provider_reference or '').strip()
        if not reference: raise ValueError('M-Pesa provider reference is required for successful settlement')
        if paid_amount is None: raise ValueError('M-Pesa paid amount is required for successful settlement')
        if Decimal(str(paid_amount)) != invoice.amount_kes: raise ValueError('M-Pesa paid amount does not match invoice amount')
