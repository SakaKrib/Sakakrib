...
def create_subscription_checkout(profile, plan_id, billing_cycle, provider, phone_number=None):
    ...
    return {'success': True, 'subscription_id': str(subscription.id), 'invoice_id': str(invoice.id),
            ...
            'provider_reference': result.provider_reference, 'provider_response': result.raw, 'status': 'PENDING'}
...
def finalize_paypal_subscription(invoice_id, order_id, capture_result):
    if not capture_result.success:
        raise ValueError(capture_result.message)
    with transaction.atomic():
        invoice = SubscriptionInvoice.objects.select_for_update().filter(pk=invoice_id).first()
        if not invoice:
        ...
        if invoice.status == 'PAID':
            return {'success': True, 'already_settled': True,
                    'subscription_id': str(invoice.landlord_subscription_id or invoice.real_estate_subscription_id)}
        return _activate_invoice(invoice, provider='PAYPAL', provider_reference=order_id, transaction_id=order_id)
...
def _activate_invoice(invoice, provider, provider_reference, transaction_id=None):
    ...
    invoice.paid_at = now
    invoice.save(update_fields=['status','payment_provider','payment_method','provider_reference','provider_transaction_id','paid_at'])
    if invoice.landlord_subscription_id:
        LandlordSubscription.objects.filter(pk=invoice.landlord_subscription_id).update(
            ...
            current_period_end=invoice.billing_period_end or now, grace_period_end=None, updated_at=now)
