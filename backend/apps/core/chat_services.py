...
from .chat_media_services import sign_chat_attachment
...
def validate_conversation_for_user(*, user_id, conversation_id):
    ...
    if conversation_id_for_users(parts[0], parts[1]) != str(conversation_id):
        raise ValidationError("Invalid conversation id")
...
def list_conversation(*, user_id, conversation_id, limit=50, before=None):
    validate_conversation_for_user(user_id=user_id, conversation_id=conversation_id)
    try:
...
def send_message(*, sender_id, receiver_id, content="", message_type="text", event_data=None):
    ...
    text = str(content or "")
    if message_type != "image" and not text.strip():
    ...
    message = ChatMessage.objects.create(conversation_id=conversation_id, sender_id=sender_id, receiver_id=receiver_id, content=text, message_type=message_type, event_data=normalized_event_data)
...
def serialize_message(message):
    event_data = message.event_data if isinstance(message.event_data, dict) else None
    if event_data and isinstance(event_data.get("attachments"), list) and event_data["attachments"]:
        first_attachment = event_data["attachments"][0]
        if isinstance(first_attachment, dict) and first_attachment.get("path"):
            attachment = dict(first_attachment)
            try:
                attachment["signed_url"] = sign_chat_attachment(message=message, user_id=message.sender_id)
            except ValidationError:
                attachment.pop("signed_url", None)
            event_data = dict(event_data)
            event_data["attachments"] = [attachment, *event_data["attachments"][1:]]
    return {
        "id": str(message.id),
