...
class MovingMpesaStartView(APIView):
    ...
    def post(self, request, booking_id):
        try:
            return Response(start_moving_mpesa_payment(renter_id=request.user.id, booking_id=booking_id))
...
class MoverPayoutCallbackView(APIView):
    ...
    def post(self, request):
        ...
        provider=payload.get("provider","MPESA")
        ...
        try:
            return Response(finalize_mover_payout(payout_id=payout_id,provider=provider,provider_reference=provider_reference,provider_transaction_id=provider_transaction_id,success=bool(success),failure_reason=payload.get("failure_reason")))
        except (ValidationError,TypeError,ValueError) as exc: return _error(exc)
