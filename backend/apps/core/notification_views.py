...
class UserNotificationView(APIView):
    ...
    def get(self, request):
        rows = list(
            UserNotification.objects.filter(user_id=request.user.pk)
            .order_by("-created_at", "-id")[:_limit(request)]
        )
    ...
    def patch(self, request):
        ...
        row = UserNotification.objects.filter(
            id=notification_id,
...
class RenterNotificationView(APIView):
    ...
    def get(self, request):
        rows = list(
            RenterNotification.objects.filter(renter_user_id=request.user.pk)
            .order_by("-created_at", "-id")[:_limit(request)]
        )
    ...
    def patch(self, request):
        ...
        if not notification_id:
            return JsonResponse({"detail": "id is required"}, status=400)
        row = RenterNotification.objects.filter(
            id=notification_id,
