...
def _invoice_number(invoice_id=None):
...
def _periods_from_payload(periods):
    ...
    for item in periods:
        ...
        seen.add((year, month)); normalized.append((year, month))
    normalized.sort()
    for (py, pm), (cy, cm) in zip(normalized, normalized[1:]):
...
def create_landlord_rent_invoice(*, landlord_id, unit_id=None, renter_assoc_id=None, periods=None, due_date=None,
    ...
    normalized = _periods_from_payload(periods)
    if due_date is None:
        raise ValidationError("Due date is required.")
    if renter_assoc_id:
        association = RenterUnitAssociation.objects.select_for_update().filter(id=renter_assoc_id, landlord_id=landlord_id, status__iexact="ACTIVE").first()
        ...
        association = None
    unit = _get_unit(unit_id, landlord_id)
    ...
    if association is None:
        ...
        if association is None:
            raise ValidationError("No active renter is associated with this unit.")
    destination = _get_payment_destination(payment_method_id, unit, landlord_id)
    period_start, period_end = _period_bounds(normalized)
    if due_date < period_start:
        raise ValidationError("Due date cannot be before the billing period starts.")
    active_ids = RentInvoice.objects.filter(status__in=["DUE", "PAYMENT_SUBMITTED"]).values_list("id", flat=True)
    for year, month in normalized:
        ...
        if RentInvoicePeriod.objects.filter(renter_assoc_id=association.id, period_year=year, period_month=month, invoice_id__in=active_ids).exists():
    ...
    dispatch_user_notification(user_id=association.renter_user_id, notification_type="RENT_INVOICE_CREATED",
        title="New rent invoice", message="A new rent invoice has been created for your unit.",
        data={"invoice_id": str(invoice.id), "invoice_number": invoice.invoice_number, "amount_kes": str(invoice.amount_kes), "due_date": due_date.isoformat()},
...
def submit_invoice_payment(*, renter_user_id, invoice_id, transaction_reference, payment_method=None, payment_date=None):
    ...
    submission = RentPaymentSubmission.objects.create(invoice_id=invoice.id, renter_user_id=renter_user_id, landlord_id=invoice.landlord_id,
    ...
    invoice.status = "PAYMENT_SUBMITTED"; invoice.transaction_reference = reference; invoice.payment_method = submission.payment_method; invoice.payment_date = payment_date
    invoice.save(update_fields=["status", "transaction_reference", "payment_method", "payment_date", "updated_at"])
    dispatch_user_notification(user_id=invoice.landlord_id, notification_type="RENT_PAYMENT_CONFIRMATION_REQUIRED",
        title="Rent payment awaiting confirmation", message="A renter has submitted a transaction ID for rent. Verify the payment externally and confirm or reject it.",
...
@transaction.atomic
def create_renter_paid_invoice(*, renter_user_id, unit_id, payment_date, payment_method, transaction_reference):
    ...
    association = RenterUnitAssociation.objects.select_for_update().filter(unit_id=unit.id, renter_user_id=renter_user_id, status__iexact="ACTIVE").first()
    if association is None:
    ...
    invoice = RentInvoice.objects.create(id=uuid.uuid4(), invoice_number=_invoice_number(), landlord_id=association.landlord_id,
        renter_user_id=renter_user_id, renter_assoc_id=association.id, listing_id=unit.listing_id, unit_id=unit.id,
        billing_period_start=period_start, billing_period_end=payment_date.replace(day=last_day), due_date=payment_date,
    ...
    dispatch_user_notification(user_id=association.landlord_id, notification_type="RENT_PAYMENT_CONFIRMATION_REQUIRED",
        title="Rent payment awaiting confirmation", message="A renter has submitted a transaction ID for rent. Verify the payment externally and confirm or reject it.",
        data={"invoice_id": str(invoice.id), "submission_id": str(submission.id), "transaction_reference": reference, "amount_kes": str(invoice.amount_kes)},
        event_key=f"rent-submission:{submission.id}")
    return {"success": True, "submission_id": str(submission.id), "invoice_id": str(invoice.id), "status": "PENDING"}
...
def confirm_rent_payment(*, landlord_id, submission_id):
    ...
    if invoice.landlord_id != landlord_id or invoice.status != "PAYMENT_SUBMITTED":
    ...
    for period in RentInvoicePeriod.objects.filter(invoice_id=invoice.id).order_by("period_year", "period_month"):
        ...
        payment = RentPayment.objects.create(renter_assoc_id=invoice.renter_assoc_id, unit_id=invoice.unit_id, landlord_id=invoice.landlord_id,
            amount_kes=period.amount_kes, period_year=period.period_year, period_month=period.period_month, status="PAID", paid_at=timezone.now(),
            payment_provider="MANUAL", payment_method=invoice.payment_method or "MANUAL", provider_reference=submission.transaction_reference,
    ...
    now = timezone.now(); submission.status = "CONFIRMED"; submission.confirmed_by = landlord_id; submission.confirmed_at = now
...
def reject_rent_payment(*, landlord_id, submission_id, rejection_reason):
    ...
    submission.status = "REJECTED"; submission.rejection_reason = reason; submission.save(update_fields=["status", "rejection_reason", "updated_at"])
    invoice.status = "REJECTED"; invoice.save(update_fields=["status", "updated_at"])
    dispatch_user_notification(user_id=invoice.renter_user_id, notification_type="RENT_PAYMENT_REJECTED", title="Rent payment needs attention",
