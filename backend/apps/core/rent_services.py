...
def _invoice_payload(invoice):
    ...
    return {
        ...
        "periods": [
            {"period_year": p.period_year, "period_month": p.period_month, "amount_kes": str(p.amount_kes)}
            for p in periods
        ],
    }
...
def _get_payment_destination(payment_method_id, unit, actor_id):
    ...
    if str(unit.user_id) != str(actor_id):
        authorized = RenterUnitAssociation.objects.filter(
            unit_id=unit.id, renter_user_id=actor_id, status__iexact="ACTIVE"
        ).exists()
        if not authorized:
            raise ValidationError("Not authorized to view this payment destination.")
    try:
        method = LandlordPaymentMethod.objects.get(
            id=payment_method_id, landlord_id=unit.user_id, is_active=True
        )
...
def _periods_from_payload(periods):
    ...
    for item in periods:
        try:
            year = int(item.get("period_year"))
            month = int(item.get("period_month"))
...
@transaction.atomic
def create_landlord_rent_invoice(*, landlord_id, unit_id=None, renter_assoc_id=None,
                                  periods=None, due_date=None, payment_method_id=None,
                                  billing_period_start=None, billing_period_end=None):
    """Create a landlord invoice using unit rent and production period rules."""
    if periods is None:
    ...
    if renter_assoc_id:
        try:
            association = RenterUnitAssociation.objects.select_for_update().get(
                id=renter_assoc_id, landlord_id=landlord_id, status__iexact="ACTIVE"
            )
    ...
    if association is None:
        association = RenterUnitAssociation.objects.select_for_update().filter(
            unit_id=unit.id, landlord_id=landlord_id, status__iexact="ACTIVE"
    ...
    for year, month in normalized_periods:
        if RentPayment.objects.filter(
            renter_assoc_id=association.id, period_year=year, period_month=month, status__iexact="PAID"
        ).exists():
            raise ValidationError(f"Billing period {year}-{month:02d} is already paid.")
        if RentInvoicePeriod.objects.filter(
            renter_assoc_id=association.id, period_year=year, period_month=month
        ).exclude(invoice__status__in=["CANCELLED", "REJECTED"]).exists():
    ...
    invoice = RentInvoice.objects.create(
        id=invoice_id,
        invoice_number=_invoice_number(invoice_id),
    ...
    RentInvoicePeriod.objects.bulk_create([
        RentInvoicePeriod(
            invoice_id=invoice.id, renter_assoc_id=association.id, unit_id=unit.id,
    ...
    dispatch_user_notification(
        ...
        notification_type="RENT_INVOICE_CREATED",
        title="New rent invoice",
        message="A new rent invoice has been created for your unit.",
        data={"invoice_id": str(invoice.id), "invoice_number": invoice.invoice_number,
              "amount_kes": str(total), "due_date": due_date.isoformat()},
...
@transaction.atomic
def submit_invoice_payment(*, renter_user_id, invoice_id, transaction_reference,
                         payment_method=None, payment_date=None):
    reference = _clean_reference(transaction_reference)
    invoice = RentInvoice.objects.select_for_update().filter(
        id=invoice_id, renter_user_id=renter_user_id
    ).first()
    if invoice is None:
    ...
    if RentPaymentSubmission.objects.filter(
        transaction_reference__iexact=reference
    ).exists():
    ...
    submission = RentPaymentSubmission.objects.create(
        ...
        renter_assoc_id=invoice.renter_assoc_id, unit_id=invoice.unit_id,
        transaction_reference=reference, payment_method=(payment_method or "").strip() or None,
    ...
    dispatch_user_notification(
        ...
        notification_type="RENT_PAYMENT_CONFIRMATION_REQUIRED",
        ...
        data={"invoice_id": str(invoice.id), "submission_id": str(submission.id),
              "transaction_reference": reference, "amount_kes": str(invoice.amount_kes)},
...
def confirm_rent_payment(*, landlord_id, submission_id):
    submission = RentPaymentSubmission.objects.select_for_update().filter(
        id=submission_id, landlord_id=landlord_id
    ).first()
    ...
    periods = RentInvoicePeriod.objects.filter(invoice_id=invoice.id).order_by("period_year", "period_month")
    payment_ids = []
    for period in periods:
        if RentPayment.objects.filter(
            renter_assoc_id=invoice.renter_assoc_id, period_year=period.period_year,
            period_month=period.period_month, status__iexact="PAID"
        ).exists():
        ...
        payment = RentPayment.objects.create(
            renter_assoc_id=invoice.renter_assoc_id, unit_id=invoice.unit_id,
            landlord_id=invoice.landlord_id, amount_kes=period.amount_kes,
            period_year=period.period_year, period_month=period.period_month, status="PAID",
            paid_at=timezone.now(), payment_provider="MANUAL",
            payment_method=invoice.payment_method or "MANUAL",
            provider_reference=submission.transaction_reference,
    ...
    data = {"invoice_id": str(invoice.id), "invoice_number": invoice.invoice_number,
            "submission_id": str(submission.id), "transaction_reference": submission.transaction_reference,
            "amount_kes": str(invoice.amount_kes), "payment_ids": payment_ids}
    dispatch_user_notification(
        user_id=invoice.renter_user_id, notification_type="RENT_PAYMENT_CONFIRMED",
        title="Rent payment confirmed",
        message="Your landlord has confirmed your rent payment. Your SakaCrib invoice is now marked PAID.",
        data=data, event_key=f"rent-confirmed:renter:{invoice.id}",
    )
    dispatch_user_notification(
        user_id=landlord_id, notification_type="RENT_PAYMENT_CONFIRMED",
        title="Rent payment confirmed",
        message="The submitted rent payment has been confirmed and recorded as PAID.",
        data=data, event_key=f"rent-confirmed:landlord:{invoice.id}",
    )
    return {"success": True, "idempotent": False, "submission_id": str(submission.id),
            "invoice_id": str(invoice.id), "invoice_number": invoice.invoice_number,
            "status": "PAID", "payment_ids": payment_ids}
...
def reject_rent_payment(*, landlord_id, submission_id, rejection_reason):
    ...
    dispatch_user_notification(
        user_id=invoice.renter_user_id,
        notification_type="RENT_PAYMENT_REJECTED",
        ...
        data={"invoice_id": str(invoice.id), "invoice_number": invoice.invoice_number,
              "submission_id": str(submission.id), "reason": reason},
