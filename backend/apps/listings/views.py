...
class ListingPaymentIntentView(APIView):
    ...
    def get(self, request, intent_id):
        ...
        return Response({
            'id': str(intent.id), 'status': intent.status,
            'listing_id': str(intent.listing_id) if intent.listing_id else None,
            'amount_kes': float(intent.amount_kes), 'provider': intent.provider,
            'provider_reference': intent.provider_reference,
            'provider_amount': float(intent.provider_amount) if intent.provider_amount is not None else None,
            'provider_currency': intent.provider_currency, 'created_at': intent.created_at,
            'paid_at': intent.paid_at, 'expires_at': intent.expires_at,
        })
...
class ListingListView(APIView):
    ...
    def _apply_filters(queryset, params):
        city = params.get('city'); county = params.get('county'); property_type = params.get('property_type')
        listing_type = params.get('listing_type'); query = params.get('q')
        ...
        if query:
            term = query.strip()
            if term:
                queryset = queryset.filter(Q(title__icontains=term) | Q(description__icontains=term) | Q(city__icontains=term) | Q(county__icontains=term) | Q(location_search__icontains=term) | Q(property_name__icontains=term) | Q(property_type__icontains=term))
...
class ListingDetailView(APIView):
    ...
    def _get_listing(self, request, listing_id):
        if not getattr(request.user, 'email_verified', False):
            return None, Response({'error': 'Email verification is required.'}, status=status.HTTP_403_FORBIDDEN)
        listing = Listing.objects.filter(pk=listing_id).first()
        if not listing:
            return None, Response({'error': 'Listing not found.'}, status=status.HTTP_404_NOT_FOUND)
        if not (is_admin(request.user) or str(listing.user_id) == str(request.user.pk) or (listing.approval_status == 'approved' and listing.is_published)):
            return None, Response({'error': 'You are not authorized to view this listing.'}, status=status.HTTP_404_NOT_FOUND)
        return listing, None
    ...
    def put(self, request, listing_id):
        return self.patch(request, listing_id)
...
class AdminListingReviewView(APIView):
    ...
    def post(self, request, listing_id):
        try:
            listing = review_listing(admin_user=request.user, listing_id=listing_id, decision=request.data.get('decision'), note=request.data.get('note', ''))
        ...
        return Response({'success': True, 'listing': ListingSerializer(listing, context={'request': request}).data})
