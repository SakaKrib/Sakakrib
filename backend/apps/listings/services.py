...
def _subscription_usage(subscription, role, user_id):
    ...
    if role == 'landlord':
        return SubscriptionListing.objects.filter(
            subscription_id=subscription.id, status='ACTIVE', listing_id__in=owned
        ).count()
    return SubscriptionListing.objects.filter(
        real_estate_subscription_id=subscription.id, status='ACTIVE', listing_id__in=owned
    ).count()
...
def get_listing_entitlement(profile):
    ...
    can_create = bool(can_start and (
        free_remaining > 0 or (subscription and (limit is None or remaining > 0))
    ))
...
def create_listing(profile, data):
    ...
    if entitlement['free_listings_remaining'] > 0:
        Profile.objects.filter(pk=profile.id).update(free_listings_used=(profile.free_listings_used or 0) + 1)
    ...
    return {
        'success': True, 'listing_created': True, 'listing_id': listing.id,
...
def create_listing_payment_intent(profile, listing_data):
    ...
    return {
        'success': True, 'payment_intent_created': True, 'listing_created': False,
