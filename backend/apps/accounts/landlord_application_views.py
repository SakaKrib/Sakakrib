...
class LandlordApplicationSubmitView(APIView):
    ...
    def post(self, request):
        ...
        if not document_url or not document_url.startswith(f'id-documents/{profile.pk}/') or '..' in Path(document_url).parts:
            return Response({'success': False, 'code': 'DOCUMENT_REQUIRED', 'message': 'Please upload your identity document before submitting.'}, status=400)
